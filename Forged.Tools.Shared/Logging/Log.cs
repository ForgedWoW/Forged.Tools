...
using Forged.Tools.Shared.Collections;
using Forged.Tools.Shared.Configuration;
using System;
...
public class Log
{
    ...
    public static void outLog(LogFilter type, LogLevel level, string text, params object[] args)
    {
        ...
        outMessage(type, level, text, AppenderType.None, args);
    }
    ...
    public static void outInfo(LogFilter type, string text, params object[] args)
    {
        ...
        outMessage(type, LogLevel.Info, text, AppenderType.None, args);
    }
    ...
    public static void outWarn(LogFilter type, string text, params object[] args)
    {
        ...
        outMessage(type, LogLevel.Warn, text, AppenderType.None, args);
    }

    public static void outNoConsoleWarn(LogFilter type, string text, params object[] args)
    {
        if (!ShouldLog(type, LogLevel.Warn))
            return;

        outMessage(type, LogLevel.Warn, text, AppenderType.Console, args);
    }
    ...
    public static void outDebug(LogFilter type, string text, params object[] args)
    {
        ...
        outMessage(type, LogLevel.Debug, text, AppenderType.None, args);
    }


    [Conditional("DEBUG")]
    public static void outNoConsoleDebug(LogFilter type, string text, params object[] args)
    {
        if (!ShouldLog(type, LogLevel.Debug))
            return;

        outMessage(type, LogLevel.Debug, text, AppenderType.Console, args);
    }


    public static void outNoConsoleError(LogFilter type, string text, params object[] args)
    {
        if (!ShouldLog(type, LogLevel.Error))
            return;

        outMessage(type, LogLevel.Error, text, AppenderType.Console, args);
    }
    ...
    public static void outError(LogFilter type, string text, params object[] args)
    {
        ...
        outMessage(type, LogLevel.Error, text, AppenderType.None, args);
    }
    ...
    public static void outException(Exception ex, [CallerMemberName]string memberName = "")
    {
        ...
        outMessage(LogFilter.Server, LogLevel.Fatal, "CallingMember: {0} ExceptionMessage: {1} \nStackTrace: {2}", AppenderType.None, memberName, ex.Message, ex.StackTrace);

        if (ex.InnerException != null)
            outException(ex.InnerException);
    }
    ...
    public static void outFatal(LogFilter type, string text, params object[] args)
    {
        ...
        outMessage(type, LogLevel.Fatal, text, AppenderType.None, args);
    }
    ...
    public static void outTrace(LogFilter type, string text, params object[] args)
    {
        ...
        outMessage(type, LogLevel.Trace, text, AppenderType.Console, args);
    }
    ...
    public static void outCommand(uint accountId, string text, params object[] args)
    {
        ...
        var msg = new LogMessage(LogLevel.Info, LogFilter.Commands, string.Format(text, args), AppenderType.None);
    ...
    static void outMessage(LogFilter type, LogLevel level, string text, AppenderType ignoredAppenderType, params object[] args)
    {
        Logger logger = GetLoggerByType(type);

        if (type == LogFilter.Sql)
            ignoredAppenderType = AppenderType.Console;

        logger.write(new LogMessage(level, type, string.Format(text, args), ignoredAppenderType));
    }
