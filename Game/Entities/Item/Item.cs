...
namespace Game.Entities
{
    public class Item : WorldObject
    {
        ...
        public void SetGem(ushort slot, ItemDynamicFieldGems gem, uint gemScalingLevel)
        {
            ...
            m_values.ModifyValue(m_itemData).ModifyValue(m_itemData.Gems, slot);
            SocketedGem gemField = new SocketedGem();
            m_itemData.Gems[slot] = gemField;
            ...
            if (source.GetSubClass() != target.GetSubClass())
            {
                switch (source.GetClass())
                {
                    ...
                    case ItemClass.Armor:
                        if ((ItemSubClassArmor)source.GetSubClass() != ItemSubClassArmor.Cosmetic && (ItemSubClassArmor)source.GetSubClass() != ItemSubClassArmor.Relic)
                            return false;
                        if (source.GetInventoryType() != target.GetInventoryType())
                            if (ItemTransmogrificationSlots[(int)source.GetInventoryType()] != ItemTransmogrificationSlots[(int)target.GetInventoryType() == (int)InventoryType.Relic ? (int)InventoryType.Robe : (int)target.GetInventoryType()])
                                return false;
                        break;
                    default:
                        return false;
