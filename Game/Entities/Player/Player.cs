...
namespace Game.Entities
{
    public partial class Player : Unit
    {
        ...
        public void GiveLevel(uint level)
        {
            ...
            packet.PowerDelta[0] = (int)GetCreateMana();
            ...
            for (CharacterStats i = CharacterStats.Strength; i < CharacterStats.Max; ++i)
            ...
            SetCreateHealth((uint)info.stats[5]);
            SetCreateMana((uint)info.stats[6]);
        ...
        public void InitStatsForLevel(bool reapplyMods = false)
        {
            ...
            SetArmor((int)(GetCreateStat(CharacterStats.Agility) * 2), 0);
            SetStatFlatModifier(UnitMods.Armor, UnitModifierFlatType.Base, (int)(GetCreateStat(CharacterStats.Agility) * 2));
        ...
        public void ForgeUIMessage(ForgeTopic topic, string message)
        {
            ...
            {
                var messageParts = message.Chunk(2048).Select(x => new string(x)).ToList();

                for (int i = 0; i < messageParts.Count; i++)
                    WhisperAddon(topic + "}" + (i + 1) + "}" + messageParts.Count + ":" + messageParts[i], SharedConst.FORGE_ADDON, false, this);
            }
            else
            {
                message = topic + ":" + message;
                WhisperAddon(message, SharedConst.FORGE_ADDON, false, this);
            }
        }
        public void WhisperAddon(string text, string prefix, bool isLogged, Player receiver)
        {
            ...
            if (SharedConst.FORGE_ADDON != prefix && !receiver.GetSession().IsAddonRegistered(prefix))
                return;
