            ...
            if (!IsGameMaster())
            {
                ...
                if (LevelMin != 0 || LevelMax != 0 || failedMapDifficultyXCondition != 0 || missingItem != 0 || missingQuest != 0 || missingAchievement != 0)
                {
                    if (report)
                    {
                        ...
                        else if (mapDiff.Message[Global.WorldMgr.GetDefaultDbcLocale()].Length != 0 && mapDiff.Message[Global.WorldMgr.GetDefaultDbcLocale()][0] != '\0' || failedMapDifficultyXCondition != 0) // if (missingAchievement) covered by this case
