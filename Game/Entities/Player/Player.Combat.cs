...
namespace Game.Entities
{
    public partial class Player
    {
        ...
        {
            GtCombatRatingsEntry Rating = CliDB.RatioCombatRatingsTable.GetRow((uint)cr*SharedConst.GTMaxLevel+GetLevel()-1);
            if (Rating == null)
                return 1.0f;

            float value = Rating.ratio;
            if (value == 0)
        ...
        float GetMeleeCritFromAgility()
        {
            ...
            GtChanceToMeleeCritBaseEntry critBase = CliDB.BaseMeleeCritChanceTable.GetRow(pclass - 1);
            GtChanceToMeleeCritEntry critRatio = CliDB.MeleeCritChanceTable.GetRow((pclass - 1) * SharedConst.GTMaxLevel + level - 1);
            if (critBase == null || critRatio == null)
                return 0.0f;

            float crit = critBase.baseRatio + GetStat(CharacterStats.Agility) * critRatio.ratio;
            return crit * 100.0f;
        }
        ...
        void GetDodgeFromAgility(ref float diminishing, ref float nondiminishing)
        {
            ...
            if (level > CliDB.MeleeCritChanceTable.GetTableRowCount())
                level = (uint) CliDB.MeleeCritChanceTable.GetTableRowCount() - 1;

            // Dodge per agility is proportional to crit per agility, which is available from DBC files
            var dodgeRatio = CliDB.MeleeCritChanceTable.GetRow((pclass - 1) * SharedConst.GTMaxLevel + level - 1);
            if (dodgeRatio == null || pclass > (int)Class.Max)
                return;

            // @todo research if talents/effects that increase total agility by x% should increase non-diminishing part
            float base_agility = GetCreateStat(CharacterStats.Agility) * GetPctModifierValue(UnitMods.StatAgility, UnitModifierPctType.Base);
            float bonus_agility = GetStat(CharacterStats.Agility) - base_agility;

            // calculate diminishing (green in char screen) and non-diminishing (white) contribution
            diminishing += 100.0f * bonus_agility * dodgeRatio.ratio * crit_to_dodge[(int)pclass - 1];
            nondiminishing += 100.0f * (dodge_base[(int)pclass - 1] + base_agility * dodgeRatio.ratio * crit_to_dodge[pclass - 1]);
        ...
        public float OCTRegenHPPerSpirit()
        {
            uint level = GetLevel();
            uint pclass = (uint)GetClass();
            ...
            GtOCTRegenHPEntry baseRatio = CliDB.OCTRegenHPTable.GetRow((pclass - 1) * SharedConst.GTMaxLevel + level - 1);
            GtRegenHPPerSptEntry moreRatio = CliDB.HPRegenPerSpiritTable.GetRow((pclass - 1) * SharedConst.GTMaxLevel + level - 1);
            if (baseRatio == null || moreRatio == null)
                return 0.0f;

            // Formula from PaperDollFrame script
            float spirit = GetStat(CharacterStats.Spirit);
        ...
        public float OCTRegenMPPerSpirit()
        {
            uint level = GetLevel();
            uint pclass = (uint)GetClass();
            ...
            GtRegenMPPerSptEntry moreRatio = CliDB.MPRegenPerSpiritTable.GetRow((pclass - 1) * SharedConst.GTMaxLevel + level - 1);
            if (moreRatio == null)
                return 0.0f;
