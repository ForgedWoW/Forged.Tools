...
namespace Game.Entities
{
    public partial class Player
    {
            ...
            for (byte i = 0; i < ItemConst.MaxStats; ++i)
            {
                ...
                switch ((ItemModType)statType)
                {
                    ...
                    case ItemModType.Agility:                          // modify agility
                        ...
                        UpdateStatBuffMod(CharacterStats.Agility);
                        break;
                    case ItemModType.Strength:                         //modify strength
                        ...
                        UpdateStatBuffMod(CharacterStats.Strength);
                        break;
                    case ItemModType.Intellect:                        //modify intellect
                        ...
                        UpdateStatBuffMod(CharacterStats.Intellect);
                        break;
                    case ItemModType.Spirit:                           //modify spirit
                        ...
                        UpdateStatBuffMod(CharacterStats.Spirit);
                        break;
                    case ItemModType.Stamina:                          //modify stamina
                        ...
                        UpdateStatBuffMod(CharacterStats.Stamina);
                        break;
        ...
        byte FindEquipSlot(Item item, uint slot, bool swap)
        {
            ...
            switch (item.GetTemplate().GetInventoryType())
            {
                ...
                case InventoryType.Relic:
                {
                    ...
                    {
                        case (int)ItemSubClassArmor.Sigil:
                            if (GetClass() == Class.Deathknight)
                                slots[0] = EquipmentSlot.Ranged;
                            break;
                        case (int)ItemSubClassArmor.Libram:
                            if (GetClass() == Class.Paladin)
                                slots[0] = EquipmentSlot.Ranged;
                            break;
                        case (int)ItemSubClassArmor.Idol:
                            if (GetClass() == Class.Druid)
                                slots[0] = EquipmentSlot.Ranged;
                            break;
                        case (int)ItemSubClassArmor.Totem:
        ...
        bool ForEachEquipmentSlot(InventoryType inventoryType, ItemTemplate item, bool canDualWield, bool canTitanGrip, EquipmentSlotDelegate callback)
        {
            switch (inventoryType)
            {
                ...
                case InventoryType.RangedRight:
                case InventoryType.Relic:
                    callback(EquipmentSlot.Ranged, true);
                    return true;
                case InventoryType.WeaponMainhand:
                    callback(EquipmentSlot.MainHand);
                    return true;
            ...
            ForEachItem(ItemSearchLocation.Everywhere, item =>
            {
                ...
                if (itemTemplate != null)
                {
                    ...
                    else if (CanEquipItem(ItemConst.NullSlot, out dest, item, true, false) == InventoryResult.Ok)
                    {
                        uint itemLevel = item.GetItemLevel(this);
                        InventoryType inventoryType = itemTemplate.GetInventoryType();
                        ForEachEquipmentSlot(inventoryType, item.GetTemplate(), m_canDualWield, m_canTitanGrip, (slot, checkDuplicateGuid) =>
                        {
