...
namespace Game.Entities
{
    public partial class Unit
    {
        ...
        CharacterStats GetStatByAuraGroup(UnitMods unitMod)
        {
            CharacterStats stat = CharacterStats.Strength;

            switch (unitMod)
            {
                case UnitMods.StatStrength:
                    stat = CharacterStats.Strength;
                    break;
                case UnitMods.StatAgility:
                    stat = CharacterStats.Agility;
                    break;
                case UnitMods.StatStamina:
                    stat = CharacterStats.Stamina;
                    break;
                case UnitMods.StatIntellect:
                    stat = CharacterStats.Intellect;
                    break;
                case UnitMods.StatSpirit:
                    stat = CharacterStats.Spirit;
                    break;
                default:
                    break;
            }
        ...
        public void UpdateStatBuffMod(CharacterStats stat)
        {
            float modPos = 0.0f;
            float modNeg = 0.0f;
            float factor = 0.0f;
        ...
        void UpdateStatBuffModForClient(CharacterStats stat)
        {
        ...
        public virtual bool UpdateStats(CharacterStats stat) { return false; }
        ...
        //Stats
        public float GetStat(CharacterStats stat) { return m_unitData.Stats[(int)stat]; }
        public void SetStat(CharacterStats stat, int val) { SetUpdateFieldValue(ref m_values.ModifyValue(m_unitData).ModifyValue(m_unitData.Stats, (int)stat), val); }
        ...
        public float GetCreateStat(CharacterStats stat)
        {
            return CreateStats[(int)stat];
        }
        public void SetCreateStat(CharacterStats stat, float val)
        {
            CreateStats[(int)stat] = val;
        }
        public float GetPosStat(CharacterStats stat) { return m_unitData.StatPosBuff[(int)stat]; }
        public float GetNegStat(CharacterStats stat) { return m_unitData.StatNegBuff[(int)stat]; }
        ...
        {
            for (var stat = CharacterStats.Strength; stat < CharacterStats.Max; ++stat)
            {
        ...
        public float GetTotalStatValue(CharacterStats stat)
        {
            UnitMods unitMod = UnitMods.StatStart + (int)stat;
        ...
        public uint GetDefenseSkillValue(Unit target = null)
        {
            if (GetTypeId() == TypeId.Player)
            {
                // in PvP use full skill instead current skill value, always max skill
                uint value = (target && target.GetTypeId() == TypeId.Player)
                               ? ToPlayer().GetMaxSkillValue(SkillType.Defense)
                               : GetLevel() * 5;
                value += (uint)ToPlayer().GetRatingBonusValue(CombatRating.DefenseSkill);
                return value;
            }
            else
    ...
    public partial class Player
    {
        ...
        {
            for (var i = CharacterStats.Strength; i < CharacterStats.Max; ++i)
            {
        ...
        public override bool UpdateStats(CharacterStats stat)
        {
            ...
            if (stat == CharacterStats.Stamina || stat == CharacterStats.Intellect || stat == CharacterStats.Strength)
            {
                Pet pet = GetPet();
                if (pet != null)
            ...
            if (stat == CharacterStats.Strength)
                UpdateAttackPowerAndDamage(false);
            else if (stat == CharacterStats.Agility)
            {
                UpdateAttackPowerAndDamage(false);
                UpdateAttackPowerAndDamage(true);
            }
        ...
        public void UpdateManaRegen()
        {
            ...
            float intellect = GetStat(CharacterStats.Intellect);
            float base_regen = (float) Math.Sqrt(intellect) * OCTRegenMPPerSpirit();
        ...
        public int GetAPPerStrength()
        {
            Class pclass = GetClass();
            return (pclass == Class.Paladin || pclass == Class.Deathknight || pclass == Class.Warrior) ? 2: 1;
        }

        public int GetAPPerAgility()
        {
            Class pclass = GetClass();
            return (pclass == Class.Rogue || pclass == Class.Shaman) ? 2 : (pclass == Class.Hunter ? 1 : 0);
        }

        public int GetRAPPerAgility()
        {
            Class pclass = GetClass();
            return (pclass == Class.Rogue || pclass == Class.Warrior) ? 1 : (pclass == Class.Hunter ? 2 : 0);
        }
        public override void UpdateAttackPowerAndDamage(bool ranged = false)
        {
            ...
            if (!HasAuraType(AuraType.OverrideAttackPowerBySpPct))
            {
                if (!ranged)
                {
                    float strengthValue = Math.Max((GetStat(CharacterStats.Strength)) * GetAPPerStrength(), 0.0f);
                    float agilityValue = Math.Max((GetStat(CharacterStats.Agility)) * GetAPPerAgility(), 0.0f);
                    ...
                    if (form != null && Convert.ToBoolean((uint)form.Flags & 0x20))
                        agilityValue += Math.Max(GetStat(CharacterStats.Agility) * GetAPPerStrength(), 0.0f);

                    val2 = strengthValue + agilityValue;
                }
                else
                    val2 = (level + Math.Max(GetStat(CharacterStats.Agility), 0.0f)) * GetRAPPerAgility();
            }
            else
            {
        ...
        uint GetShieldBlockValue()
        {
            float blockScale = 0.5f;

            var blockScaleAuras = GetAuraEffectsByType(AuraType.ModShieldBlockvaluePct);
            foreach(var effect in blockScaleAuras)
                blockScale += effect.GetAmount() * 0.01f;

            float value = (GetBaseModValue(BaseModGroup.ShieldBlockValue, BaseModType.FlatMod) + GetStat(CharacterStats.Strength) * blockScale - 10) * GetBaseModValue(BaseModGroup.ShieldBlockValue, BaseModType.PctMod);
        ...
        public override void UpdateArmor()
        {
            ...
            GetTotalAuraModifier(AuraType.ModArmorPctFromStat, aurEff =>
            {
                ...
                CharacterStats stat = (miscValue != -2) ? (CharacterStats)miscValue : GetPrimaryStat();

                value += MathFunctions.CalculatePct((float)GetStat(stat), aurEff.GetAmount());
                return true;
            });
            value += 2*(GetStat(CharacterStats.Agility) - GetCreateStat(CharacterStats.Agility));
            float baseValue = value;
        ...
        public void UpdateDefenseBonusesMod()
        {
            UpdateBlockPercentage();
            UpdateParryPercentage();
            UpdateDodgePercentage();
        ...
        CharacterStats GetPrimaryStat()
        {
            ...
            return CharacterStats.Intellect;
        }
        ...
        float GetHealthBonusFromStamina()
        {
            ...
            float stamina = GetStat(CharacterStats.Stamina);
        ...
        public float GetManaBonusFromIntellect() 
        {
            float intellect = GetStat(CharacterStats.Intellect);
            float base_int = intellect < 20 ? intellect : 20;
            float more_int = intellect - base_int;
            return base_int + (more_int * 15.0f);
        }
