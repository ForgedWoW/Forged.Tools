    ...
    public class Guardian : Minion
    {
        ...
        public bool InitStatsForLevel(uint petlevel)
        {
            ...
            if (pInfo != null)                                      // exist in DB
            {
                ...
                for (byte stat = 0; stat < (int)CharacterStats.Max; ++stat)
                    SetCreateStat((CharacterStats)stat, pInfo.stats[stat]);
            }
            else                                            // not exist in DB, use some default fake data
            {
                ...
                SetCreateStat(CharacterStats.Strength, 22);
                SetCreateStat(CharacterStats.Agility, 22);
                SetCreateStat(CharacterStats.Stamina, 25);
                SetCreateStat(CharacterStats.Intellect, 28);
            }
                ...
                default:
                {
                    switch (GetEntry())
                    {
                        ...
                        case 29264: // Feral Spirit
                        {
                            ...
                            SetStatFlatModifier(UnitMods.StatStamina, UnitModifierFlatType.Base, GetOwner().GetStat(CharacterStats.Stamina) * 0.3f);  // Bonus Stamina (30% of player stamina)
        ...
        public override bool UpdateStats(CharacterStats stat)
        {
            ...
            float mod = 0.75f;
            if (IsPetGhoul() && (stat == CharacterStats.Stamina || stat == CharacterStats.Strength))
            {
                switch (stat)
                {
            ...
            else if (stat == CharacterStats.Stamina)
            {
                ownersBonus = MathFunctions.CalculatePct(owner.GetStat(CharacterStats.Stamina), 30);
                value += ownersBonus;
            }
            //warlock's and mage's pets gain 30% of owner's intellect
            else if (stat == CharacterStats.Intellect)
            {
        ...
        public override void UpdateMaxHealth()
        {
            ...
            float stamina = GetStat(CharacterStats.Stamina) - GetCreateStat(CharacterStats.Stamina);
        ...
        public override void UpdateAttackPowerAndDamage(bool ranged = false)
        {
            ...
            if (GetEntry() == ENTRY_IMP)                                   // imp's attack power
                val = GetStat(CharacterStats.Strength) - 10.0f;
            else
                val = 2 * GetStat(CharacterStats.Strength) - 20.0f;
        ...
        public float GetBonusStatFromOwner(CharacterStats stat) { return m_statFromOwner[(int)stat]; }

        int m_bonusSpellDamage;
        float[] m_statFromOwner = new float[(int)CharacterStats.Max];
    }
