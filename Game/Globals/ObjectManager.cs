...
namespace Game
{
    public sealed class ObjectManager : Singleton<ObjectManager>
    {
        ...
        public void LoadRaceAndClassExpansionRequirements()
        {
            ...
            if (!result.IsEmpty())
            {
                ...
                do
                {
                    ...
                    if (raceEntry == null)
                    {
                        Log.outWarn(LogFilter.Sql, "Race {0} defined in `race_unlock_requirement` does not exists, skipped.", raceID);
                        continue;
                    }

                    if (expansion >= (int)Expansion.MaxAccountExpansions)
                    {
                        Log.outWarn(LogFilter.Sql, "Race {0} defined in `race_unlock_requirement` has incorrect expansion {1}, skipped.", raceID, expansion);
                        continue;
                    }

                    if (achievementId != 0 && !CliDB.AchievementStorage.ContainsKey(achievementId))
                    {
                        Log.outWarn(LogFilter.Sql, $"Race {raceID} defined in `race_unlock_requirement` has incorrect achievement {achievementId}, skipped.");
                        continue;
                    }
            ...
            if (!result.IsEmpty())
            {
                ...
                do
                {
                    ...
                    if (classEntry == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Class {classID} (race {raceID}) defined in `class_expansion_requirement` does not exists, skipped.");
                        continue;
                    }
                    ...
                    if (raceEntry == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Race {raceID} (class {classID}) defined in `class_expansion_requirement` does not exists, skipped.");
                        continue;
                    }

                    if (activeExpansionLevel >= (int)Expansion.Max)
                    {
                        Log.outWarn(LogFilter.Sql, $"Class {classID} Race {raceID} defined in `class_expansion_requirement` has incorrect ActiveExpansionLevel {activeExpansionLevel}, skipped.");
                        continue;
                    }

                    if (accountExpansionLevel >= (int)Expansion.MaxAccountExpansions)
                    {
                        Log.outWarn(LogFilter.Sql, $"Class {classID} Race {raceID} defined in `class_expansion_requirement` has incorrect AccountExpansionLevel {accountExpansionLevel}, skipped.");
                        continue;
                    }
        ...
        public string GetCypherString(uint entry, Locale locale = Locale.enUS)
        {
            ...
            {
                Log.outWarn(LogFilter.Sql, "Cypher string entry {0} not found in DB.", entry);
                return "<Error>";
            }
        ...
        public void LoadGossipMenu()
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Table gossip_menu: Id {0} is using non-existing TextId {1}", gMenu.MenuId, gMenu.TextId);
                    continue;
                }
        ...
        public void LoadGossipMenuItems()
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"Table `gossip_menu_option` for menu {gMenuItem.MenuId}, id {gMenuItem.OptionId} has unknown NPC option id {gMenuItem.OptionNpc}. Replacing with GossipOptionNpc.None");
                ...
                if (gMenuItem.OptionBroadcastTextId != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `gossip_menu_option` for MenuId {gMenuItem.MenuId}, OptionIndex {gMenuItem.OptionId} has non-existing or incompatible OptionBroadcastTextId {gMenuItem.OptionBroadcastTextId}, ignoring.");
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"Table `gossip_menu_option` for menu {gMenuItem.MenuId}, id {gMenuItem.OptionId} use non-existing Language {gMenuItem.Language}, ignoring");
                    gMenuItem.Language = 0;
                }

                if (gMenuItem.ActionMenuId != 0 && gMenuItem.OptionNpc != GossipOptionNpc.None)
                {
                    Log.outWarn(LogFilter.Sql, $"Table `gossip_menu_option` for menu {gMenuItem.MenuId}, id {gMenuItem.OptionId} can not use ActionMenuID for GossipOptionNpc different from GossipOptionNpc.None, ignoring");
                ...
                if (gMenuItem.ActionPoiId != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `gossip_menu_option` for menu {gMenuItem.MenuId}, id {gMenuItem.OptionId} can not use ActionPoiID for GossipOptionNpc different from GossipOptionNpc.None, ignoring");
                        gMenuItem.ActionPoiId = 0;
                    }
                    else if (GetPointOfInterest(gMenuItem.ActionPoiId) == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `gossip_menu_option` for menu {gMenuItem.MenuId}, id {gMenuItem.OptionId} use non-existing ActionPoiID {gMenuItem.ActionPoiId}, ignoring");
                        gMenuItem.ActionPoiId = 0;
                    }
                }
                ...
                if (gMenuItem.BoxBroadcastTextId != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `gossip_menu_option` for MenuId {gMenuItem.MenuId}, OptionIndex {gMenuItem.OptionId} has non-existing or incompatible BoxBroadcastTextId {gMenuItem.BoxBroadcastTextId}, ignoring.");
        ...
        public void LoadGossipMenuFriendshipFactions()
        {
            ...
            do
            {
                ...
                if (faction != null)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, $"Table gossip_menu_addon: ID {menuID} is using FriendshipFactionID {addon.FriendshipFactionID} referencing non-existing FriendshipRepID {faction.FriendshipRepID}");
                        addon.FriendshipFactionID = 0;
                    }
                }
                else
                {
                    Log.outWarn(LogFilter.Sql, $"Table gossip_menu_addon: ID {menuID} is using non-existing FriendshipFactionID {addon.FriendshipFactionID}");
                    addon.FriendshipFactionID = 0;
                }
        ...
        public void LoadPointsOfInterest()
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"Table `points_of_interest` (ID: {id}) have invalid coordinates (PositionX: {POI.Pos.X} PositionY: {POI.Pos.Y} PositionZ: {POI.Pos.Z}), ignored.");
                    continue;
                }
        ...
        public void LoadGraveyardZones()
        {
            ...
            do
            {
                ...
                if (entry == null)
                {
                    Log.outWarn(LogFilter.Sql, "Table `graveyard_zone` has a record for not existing graveyard (WorldSafeLocs.dbc id) {0}, skipped.", safeLocId);
                    continue;
                }
                ...
                if (areaEntry == null)
                {
                    Log.outWarn(LogFilter.Sql, "Table `graveyard_zone` has a record for not existing zone id ({0}), skipped.", zoneId);
                    continue;
                }

                if (team != 0 && team != Team.Horde && team != Team.Alliance)
                {
                    Log.outWarn(LogFilter.Sql, "Table `graveyard_zone` has a record for non player faction ({0}), skipped.", team);
                    continue;
                }

                if (!AddGraveYardLink(safeLocId, zoneId, team, false))
                    Log.outWarn(LogFilter.Sql, "Table `graveyard_zone` has a duplicate record for Graveyard (ID: {0}) and Zone (ID: {1}), skipped.", safeLocId, zoneId);
            } while (result.NextRow());
        ...
        public void LoadWorldSafeLocs()
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"World location (ID: {id}) has a invalid position MapID: {loc.GetMapId()} {loc}, skipped");
                    continue;
                }
        ...
        public WorldSafeLocsEntry GetClosestGraveYard(WorldLocation location, Team team, WorldObject conditionObject)
        {
            ...
            if (range.Empty() && !mapEntry.IsBattlegroundOrArena())
            {
                if (zoneId != 0) // zone == 0 can't be fixed, used by bliz for bugged zones
                    Log.outWarn(LogFilter.Sql, "Table `game_graveyard_zone` incomplete: Zone {0} Team {1} does not have a linked graveyard.", zoneId, team);
                return GetDefaultGraveYard(team);
            }
            ...
            foreach (var data in range)
            {
                ...
                if (entry == null)
                {
                    Log.outWarn(LogFilter.Sql, "Table `game_graveyard_zone` has record for not existing graveyard (WorldSafeLocs.dbc id) {0}, skipped.", data.safeLocId);
                    continue;
                }
        ...
        public void LoadAreaTriggerScripts()
        {
            ...
            do
            {
                ...
                if (atEntry == null)
                {
                    Log.outWarn(LogFilter.Sql, "Area trigger (Id:{0}) does not exist in `AreaTrigger.dbc`.", id);
                    continue;
                }
                ++count;
            ...
            do
            {
                ...
                switch (tmp.command)
                {
                    case ScriptCommands.Talk:
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` has invalid talk type (datalong = {1}) in SCRIPT_COMMAND_TALK for script id {2}",
                                tableName, tmp.Talk.ChatType, tmp.id);
                            continue;
                        }
                        if (!CliDB.BroadcastTextStorage.ContainsKey((uint)tmp.Talk.TextID))
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` has invalid talk text id (dataint = {1}) in SCRIPT_COMMAND_TALK for script id {2}",
                    ...
                    case ScriptCommands.Emote:
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` has invalid emote id (datalong = {1}) in SCRIPT_COMMAND_EMOTE for script id {2}",
                    ...
                    case ScriptCommands.TeleportTo:
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` has invalid map (Id: {1}) in SCRIPT_COMMAND_TELEPORT_TO for script id {2}",
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` has invalid coordinates (X: {1} Y: {2} Z: {3} O: {4}) in SCRIPT_COMMAND_TELEPORT_TO for script id {5}",
                    ...
                    case ScriptCommands.QuestExplored:
                    {
                        ...
                        if (quest == null)
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` has invalid quest (ID: {1}) in SCRIPT_COMMAND_QUEST_EXPLORED in `datalong` for script id {2}",
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` has quest (ID: {1}) in SCRIPT_COMMAND_QUEST_EXPLORED in `datalong` for script id {2}, but quest not have flag QUEST_SPECIAL_FLAGS_EXPLORATION_OR_EVENT in quest flags. Script command or quest flags wrong. Quest modified to require objective.",
                                tableName, tmp.QuestExplored.QuestID, tmp.id);
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` has too large distance ({1}) for exploring objective complete in `datalong2` in SCRIPT_COMMAND_QUEST_EXPLORED in `datalong` for script id {2}",
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` has too large distance ({1}) for exploring objective complete in `datalong2` in SCRIPT_COMMAND_QUEST_EXPLORED in `datalong` for script id {2}, max distance is {3} or 0 for disable distance check",
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` has too small distance ({1}) for exploring objective complete in `datalong2` in SCRIPT_COMMAND_QUEST_EXPLORED in `datalong` for script id {2}, min distance is {3} or 0 for disable distance check",
                    ...
                    case ScriptCommands.KillCredit:
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` has invalid creature (Entry: {1}) in SCRIPT_COMMAND_KILL_CREDIT for script id {2}",
                    ...
                    case ScriptCommands.RespawnGameobject:
                    {
                        ...
                        if (data == null)
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` has invalid gameobject (GUID: {1}) in SCRIPT_COMMAND_RESPAWN_GAMEOBJECT for script id {2}",
                        ...
                        if (info == null)
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` has gameobject with invalid entry (GUID: {1} Entry: {2}) in SCRIPT_COMMAND_RESPAWN_GAMEOBJECT for script id {3}",
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` have gameobject type ({1}) unsupported by command SCRIPT_COMMAND_RESPAWN_GAMEOBJECT for script id {2}",
                    ...
                    case ScriptCommands.TempSummonCreature:
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` has invalid coordinates (X: {1} Y: {2} Z: {3} O: {4}) in SCRIPT_COMMAND_TEMP_SUMMON_CREATURE for script id {5}",
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` has invalid creature (Entry: {1}) in SCRIPT_COMMAND_TEMP_SUMMON_CREATURE for script id {2}",
                    ...
                    case ScriptCommands.CloseDoor:
                    {
                        ...
                        if (data == null)
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` has invalid gameobject (GUID: {1}) in {2} for script id {3}",
                        ...
                        if (info == null)
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` has gameobject with invalid entry (GUID: {1} Entry: {2}) in {3} for script id {4}",
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` has gameobject type ({1}) non supported by command {2} for script id {3}",
                    ...
                    case ScriptCommands.RemoveAura:
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` using non-existent spell (id: {1}) in SCRIPT_COMMAND_REMOVE_AURA for script id {2}",
                                tableName, tmp.RemoveAura.SpellID, tmp.id);
                            continue;
                        }
                        if (Convert.ToBoolean((int)tmp.RemoveAura.Flags & ~0x1))                    // 1 bits (0, 1)
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` using unknown flags in datalong2 ({1}) in SCRIPT_COMMAND_REMOVE_AURA for script id {2}",
                    ...
                    case ScriptCommands.CastSpell:
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` using non-existent spell (id: {1}) in SCRIPT_COMMAND_CAST_SPELL for script id {2}",
                                tableName, tmp.CastSpell.SpellID, tmp.id);
                            continue;
                        }
                        if ((int)tmp.CastSpell.Flags > 4)                      // targeting type
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` using unknown target in datalong2 ({1}) in SCRIPT_COMMAND_CAST_SPELL for script id {2}",
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` using unknown flags in dataint ({1}) in SCRIPT_COMMAND_CAST_SPELL for script id {2}",
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` using invalid creature entry in dataint ({1}) in SCRIPT_COMMAND_CAST_SPELL for script id {2}",
                                tableName, tmp.CastSpell.CreatureEntry, tmp.id);
                            continue;
                        }
                        break;
                    ...
                    case ScriptCommands.CreateItem:
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` has nonexistent item (entry: {1}) in SCRIPT_COMMAND_CREATE_ITEM for script id {2}",
                                tableName, tmp.CreateItem.ItemEntry, tmp.id);
                            continue;
                        }
                        if (tmp.CreateItem.Amount == 0)
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` SCRIPT_COMMAND_CREATE_ITEM but amount is {1} for script id {2}",
                    ...
                    case ScriptCommands.PlayAnimkit:
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `{0}` has invalid AnimKid id (datalong = {1}) in SCRIPT_COMMAND_PLAY_ANIMKIT for script id {2}",
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `{tableName}` uses deprecated direct updatefield modify command {tmp.command} for script id {tmp.id}");
                        continue;
                    }
                    default:
                        break;
        ...
        public void LoadSpellScripts()
        {
            ...
            foreach (var script in sSpellScripts)
            {
                ...
                if (spellInfo == null)
                {
                    Log.outWarn(LogFilter.Sql, "Table `spell_scripts` has not existing spell (Id: {0}) as script id", spellId);
                    continue;
                }
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"Table `spell_scripts` has too high effect index {spellEffIndex} for spell (Id: {spellId}) as script id");
                    continue;
                }
                ...
                if (spellInfo.GetEffect(spellEffIndex).Effect == 0 || (spellInfo.GetEffect(spellEffIndex).Effect != SpellEffectName.ScriptEffect && spellInfo.GetEffect(spellEffIndex).Effect != SpellEffectName.Dummy))
                    Log.outWarn(LogFilter.Sql, $"Table `spell_scripts` - spell {spellId} effect {spellEffIndex} is not SPELL_EFFECT_SCRIPT_EFFECT or SPELL_EFFECT_DUMMY");
            }
        }
        public void LoadEventScripts()
        {
            ...
            foreach (var script in sEventScripts)
            {
                ...
                if (id == 0)
                    Log.outWarn(LogFilter.Sql, "Table `event_scripts` has script (Id: {0}) not referring to any gameobject_template type 10 data2 field, type 3 data6 field, type 13 data 2 field or any spell effect {1}",
        ...
        public void LoadWaypointScripts()
        {
            ...
            foreach (var id in actionSet)
                Log.outWarn(LogFilter.Sql, "There is no waypoint which links to the waypoint script {0}", id);
        }
        public void LoadSpellScriptNames()
        {
            ...
            do
            {
                ...
                if (spellInfo == null)
                {
                    Log.outWarn(LogFilter.Sql, "Scriptname: `{0}` spell (Id: {1}) does not exist.", scriptName, spellId);
                    continue;
                }
                ...
                if (allRanks)
                {
                    if (!spellInfo.IsRanked())
                        Log.outWarn(LogFilter.Sql, "Scriptname: `{0}` spell (Id: {1}) has no ranks of spell.", scriptName, spellId);

                    if (spellInfo.GetFirstRankSpell().Id != spellId)
                    {
                        Log.outWarn(LogFilter.Sql, "Scriptname: `{0}` spell (Id: {1}) is not first rank of spell.", scriptName, spellId);
                        continue;
                    }
                ...
                {
                    if (spellInfo.IsRanked())
                        Log.outWarn(LogFilter.Sql, "Scriptname: `{0}` spell (Id: {1}) is ranked spell. Perhaps not all ranks are assigned to this script.", scriptName, spellId);
        ...
        void LoadCreatureTemplateResistances()
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"creature_template_resistance has resistance definitions for creature {creatureID} but this school {school} doesn't exist");
                    continue;
                }

                if (!creatureTemplateStorage.TryGetValue(creatureID, out CreatureTemplate creatureTemplate))
                {
                    Log.outWarn(LogFilter.Sql, $"creature_template_resistance has resistance definitions for creature {creatureID} but this creature doesn't exist");
                    continue;
                }
        ...
        void LoadCreatureTemplateSpells()
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"creature_template_spell has spell definitions for creature {creatureID} with a incorrect index {index}");
                    continue;
                }

                if (!creatureTemplateStorage.TryGetValue(creatureID, out CreatureTemplate creatureTemplate))
                {
                    Log.outWarn(LogFilter.Sql, $"creature_template_spell has spell definitions for creature {creatureID} but this creature doesn't exist");
                    continue;
                }
        ...
        void LoadCreatureTemplateModels()
        {
            ...
            do
            {
                ...
                if (cInfo == null)
                {
                    Log.outWarn(LogFilter.Sql, $"Creature template (Entry: {creatureId}) does not exist but has a record in `creature_template_model`");
                    continue;
                }
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"Creature (Entry: {creatureId}) lists non-existing CreatureDisplayID id ({creatureDisplayId}), this can crash the client.");
                    continue;
                }

                CreatureModelInfo modelInfo = GetCreatureModelInfo(creatureDisplayId);
                if (modelInfo == null)
                    Log.outNoConsoleError(LogFilter.Sql, $"No model data exist for `CreatureDisplayID` = {creatureDisplayId} listed by creature (Entry: {creatureId}).");
        ...
        void LoadCreatureSummonedData()
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"Table `creature_summoned_data` references non-existing creature {creatureId}, skipped");
                    continue;
                }
                ...
                if (!result.IsNull(1))
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `creature_summoned_data` references non-existing creature {summonedData.CreatureIDVisibleToSummoner.Value} in CreatureIDVisibleToSummoner for creature {creatureId}, set to 0");
                ...
                if (!result.IsNull(2))
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `creature_summoned_data` references non-existing display id {summonedData.GroundMountDisplayID.Value} in GroundMountDisplayID for creature {creatureId}, set to 0");
                        summonedData.CreatureIDVisibleToSummoner = null;
                    }
                }
                ...
                if (!result.IsNull(3))
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `creature_summoned_data` references non-existing display id {summonedData.FlyingMountDisplayID.Value} in FlyingMountDisplayID for creature {creatureId}, set to 0");
        ...
        public void LoadCreatureTemplateAddons()
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Creature template (Entry: {0}) does not exist but has a record in `creature_template_addon`", entry);
                    continue;
                }
                ...
                for (var c = 0; c < tokens.Length; ++c)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) has wrong spell {1} defined in `auras` field in `creature_template_addon`.", entry, spellId);
                        continue;
                    }

                    if (AdditionalSpellInfo.HasAura(AuraType.ControlVehicle))
                        Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) has SPELL_AURA_CONTROL_VEHICLE aura {1} defined in `auras` field in `creature_template_addon`.", entry, spellId);

                    if (creatureAddon.auras.Contains(spellId))
                    {
                        Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) has duplicate aura (spell {1}) in `auras` field in `creature_template_addon`.", entry, spellId);
                        continue;
                    }

                    if (AdditionalSpellInfo.GetDuration() > 0)
                    {
                        Log.outWarn(LogFilter.Sql, $"Creature (Entry: {entry}) has temporary aura (spell {spellId}) in `auras` field in `creature_template_addon`.");
                        continue;
                    }
                ...
                if (creatureAddon.mount != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) has invalid displayInfoId ({1}) for mount defined in `creature_template_addon`", entry, creatureAddon.mount);
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) has invalid emote ({1}) defined in `creatureaddon`.", entry, creatureAddon.emote);
                    creatureAddon.emote = 0;
                }

                if (creatureAddon.aiAnimKit != 0 && !CliDB.AnimKitStorage.ContainsKey(creatureAddon.aiAnimKit))
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) has invalid aiAnimKit ({1}) defined in `creature_template_addon`.", entry, creatureAddon.aiAnimKit);
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) has invalid movementAnimKit ({1}) defined in `creature_template_addon`.", entry, creatureAddon.movementAnimKit);
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) has invalid meleeAnimKit ({1}) defined in `creature_template_addon`.", entry, creatureAddon.meleeAnimKit);
                    creatureAddon.meleeAnimKit = 0;
                }

                if (creatureAddon.visibilityDistanceType >= VisibilityDistanceType.Max)
                {
                    Log.outWarn(LogFilter.Sql, $"Creature (Entry: {entry}) has invalid visibilityDistanceType ({creatureAddon.visibilityDistanceType}) defined in `creature_template_addon`.");
        ...
        public void LoadCreatureAddons()
        {
            ...
            do
            {
                ...
                if (creData == null)
                {
                    Log.outWarn(LogFilter.Sql, "Creature (GUID: {0}) does not exist but has a record in `creatureaddon`", guid);
                    continue;
                }
                ...
                if (creData.movementType == (byte)MovementGeneratorType.Waypoint && creatureAddon.path_id == 0)
                {
                    ...
                    Log.outWarn(LogFilter.Sql, "Creature (GUID {0}) has movement type set to WAYPOINTMOTIONTYPE but no path assigned", guid);
                }
                ...
                for (var c = 0; c < tokens.Length; ++c)
                {
                    ...
                    if (AdditionalSpellInfo == null)
                    {
                        Log.outWarn(LogFilter.Sql, "Creature (GUID: {0}) has wrong spell {1} defined in `auras` field in `creatureaddon`.", guid, spellId);
                        continue;
                    }

                    if (AdditionalSpellInfo.HasAura(AuraType.ControlVehicle))
                        Log.outWarn(LogFilter.Sql, "Creature (GUID: {0}) has SPELL_AURA_CONTROL_VEHICLE aura {1} defined in `auras` field in `creature_addon`.", guid, spellId);

                    if (creatureAddon.auras.Contains(spellId))
                    {
                        Log.outWarn(LogFilter.Sql, "Creature (GUID: {0}) has duplicate aura (spell {1}) in `auras` field in `creature_addon`.", guid, spellId);
                        continue;
                    }

                    if (AdditionalSpellInfo.GetDuration() > 0)
                    {
                        Log.outWarn(LogFilter.Sql, $"Creature (GUID: {guid}) has temporary aura (spell {spellId}) in `auras` field in `creature_addon`.");
                        continue;
                    }
                ...
                if (creatureAddon.mount != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Creature (GUID: {0}) has invalid displayInfoId ({1}) for mount defined in `creatureaddon`", guid, creatureAddon.mount);
                        creatureAddon.mount = 0;
                    }
                }
                ...
                if (!CliDB.EmotesStorage.ContainsKey(creatureAddon.emote))
                {
                    Log.outWarn(LogFilter.Sql, "Creature (GUID: {0}) has invalid emote ({1}) defined in `creatureaddon`.", guid, creatureAddon.emote);
                    creatureAddon.emote = 0;
                }
                ...
                if (creatureAddon.aiAnimKit != 0 && !CliDB.AnimKitStorage.ContainsKey(creatureAddon.aiAnimKit))
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Guid: {0}) has invalid aiAnimKit ({1}) defined in `creature_addon`.", guid, creatureAddon.aiAnimKit);
                    creatureAddon.aiAnimKit = 0;
                }

                if (creatureAddon.movementAnimKit != 0 && !CliDB.AnimKitStorage.ContainsKey(creatureAddon.movementAnimKit))
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Guid: {0}) has invalid movementAnimKit ({1}) defined in `creature_addon`.", guid, creatureAddon.movementAnimKit);
                    creatureAddon.movementAnimKit = 0;
                }

                if (creatureAddon.meleeAnimKit != 0 && !CliDB.AnimKitStorage.ContainsKey(creatureAddon.meleeAnimKit))
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Guid: {0}) has invalid meleeAnimKit ({1}) defined in `creature_addon`.", guid, creatureAddon.meleeAnimKit);
                    creatureAddon.meleeAnimKit = 0;
                }

                if (creatureAddon.visibilityDistanceType >= VisibilityDistanceType.Max)
                {
                    Log.outWarn(LogFilter.Sql, $"Creature (GUID: {guid}) has invalid visibilityDistanceType ({creatureAddon.visibilityDistanceType}) defined in `creature_addon`.");
                    creatureAddon.visibilityDistanceType = VisibilityDistanceType.Normal;
                }
        ...
        public void LoadCreatureQuestItems()
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Table `creature_questitem` has data for nonexistent creature (entry: {0}, idx: {1}), skipped", entry, idx);
                    continue;
                }

                if (!CliDB.ItemStorage.ContainsKey(item))
                {
                    Log.outWarn(LogFilter.Sql, "Table `creature_questitem` has nonexistent item (ID: {0}) in creature (entry: {1}, idx: {2}), skipped", item, entry, idx);
                    continue;
                }
        ...
        public void LoadEquipmentTemplates()
        {
            ...
            do
            {
                ...
                if (GetCreatureTemplate(entry) == null)
                {
                    Log.outWarn(LogFilter.Sql, "Creature template (CreatureID: {0}) does not exist but has a record in `creature_equip_template`", entry);
                    continue;
                }
                ...
                for (var i = 0; i < SharedConst.MaxEquipmentItems; ++i)
                {
                    ...
                    if (dbcItem == null)
                    {
                        Log.outWarn(LogFilter.Sql, "Unknown item (ID: {0}) in creature_equip_template.ItemID{1} for CreatureID  = {2}, forced to 0.",
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Unknown item appearance for (ID: {0}, AppearanceModID: {1}) pair in creature_equip_template.ItemID{2} creature_equip_template.AppearanceModID{3} " +
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Item (ID {0}) in creature_equip_template.ItemID{1} for CreatureID  = {2} is not equipable in a hand, forced to 0.",
                            equipmentInfo.Items[i].ItemId, i + 1, entry);
                        equipmentInfo.Items[i].ItemId = 0;
                    }
                }
        ...
        public void LoadCreatureMovementOverrides()
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"Creature (GUID: {spawnId}) does not exist but has a record in `creature_movement_override`");
                    continue;
                }
        ...
        public void LoadCreatureClassLevelStats()
        {
            ...
            do
            {
                ...
                if (_class == 0 || ((1 << (_class - 1)) & (int)Class.ClassMaskAllCreatures) == 0)
                    Log.outWarn(LogFilter.Sql, "Creature base stats for level {0} has invalid class {1}", Level, _class);
            ...
            foreach (var creatureTemplate in creatureTemplateStorage.Values)
            {
                for (short lvl = creatureTemplate.Minlevel; lvl <= creatureTemplate.Maxlevel; ++lvl)
                {
                    if (creatureBaseStatsStorage.LookupByKey(MathFunctions.MakePair16((uint)lvl, creatureTemplate.UnitClass)) == null)
                        Log.outWarn(LogFilter.Sql, "Missing base stats for creature class {0} level {1}", creatureTemplate.UnitClass, lvl);
                }
            }
        ...
        public void LoadCreatureModelInfo()
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Table `creature_model_info` has a non-existent DisplayID (ID: {0}). Skipped.", displayId);
                    continue;
                }
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Table `creature_model_info` has a non-existent DisplayID_Other_Gender (ID: {0}) being used by DisplayID (ID: {1}).", modelInfo.DisplayIdOtherGender, displayId);
        ...
        public void LoadCreatureScalingData()
        {
            ...
            do
            {
                ...
                if (template == null)
                {
                    Log.outWarn(LogFilter.Sql, $"Creature template (Entry: {entry}) does not exist but has a record in `creature_template_scaling`");
                    continue;
                }
        ...
        public void CheckCreatureTemplate(CreatureTemplate cInfo)
        {
            ...
            for (uint diff = 0; diff < SharedConst.MaxCreatureDifficulties && ok; ++diff)
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) has `difficulty_entry_{1}`={2} but creature entry {3} does not exist.",
                ...
                for (uint diff2 = 0; diff2 < SharedConst.MaxCreatureDifficulties && ok2; ++diff2)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) is listed as `difficulty_entry_{1}` of another creature, but itself lists {2} in `difficulty_entry_{3}`.",
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) already listed as `difficulty_entry_{1}` for another entry.", cInfo.DifficultyEntry[diff], diff2 + 1);
                        continue;
                    }

                    if (hasDifficultyEntries[diff2].Contains(cInfo.DifficultyEntry[diff]))
                    {
                        Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) has `difficulty_entry_{1}`={2} but creature entry {3} has itself a value in `difficulty_entry_{4}`.",
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Id: {0}, Expansion {1}) has different `HealthScalingExpansion` in difficulty {2} mode (Id: {3}, Expansion: {4}).",
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}, minlevel: {1}) has lower `minlevel` in difficulty {2} mode (Entry: {3}, minlevel: {4}).",
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}, maxlevel: {1}) has lower `maxlevel` in difficulty {2} mode (Entry: {3}, maxlevel: {4}).",
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}, faction: {1}) has different `faction` in difficulty {2} mode (Entry: {3}, faction: {4}).",
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}, class: {1}) has different `unit_class` in difficulty {2} mode (Entry: {3}, class: {4}).",
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) has different `npcflag` in difficulty {1} mode (Entry: {2}).", cInfo.Entry, diff + 1, cInfo.DifficultyEntry[diff]);
                    Log.outWarn(LogFilter.Sql, "Possible FIX: UPDATE `creature_template` SET `npcflag`=`npcflag`^{0} WHERE `entry`={1};", cInfo.Npcflag ^ difficultyInfo.Npcflag, cInfo.DifficultyEntry[diff]);
                    continue;
                }

                if (cInfo.DmgSchool != difficultyInfo.DmgSchool)
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}, `dmgschool`: {1}) has different `dmgschool` in difficulty {2} mode (Entry: {3}, `dmgschool`: {4}).",
                        cInfo.Entry, cInfo.DmgSchool, diff + 1, cInfo.DifficultyEntry[diff], difficultyInfo.DmgSchool);
                    Log.outWarn(LogFilter.Sql, "Possible FIX: UPDATE `creature_template` SET `dmgschool`={0} WHERE `entry`={1};", cInfo.DmgSchool, cInfo.DifficultyEntry[diff]);
                }

                if (cInfo.UnitFlags2 != difficultyInfo.UnitFlags2)
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}, `unit_flags2`: {1}) has different `unit_flags2` in difficulty {2} mode (Entry: {3}, `unit_flags2`: {4}).",
                        cInfo.Entry, cInfo.UnitFlags2, diff + 1, cInfo.DifficultyEntry[diff], difficultyInfo.UnitFlags2);
                    Log.outWarn(LogFilter.Sql, "Possible FIX: UPDATE `creature_template` SET `unit_flags2`=`unit_flags2`^{0} WHERE `entry`={1};", cInfo.UnitFlags2 ^ difficultyInfo.UnitFlags2, cInfo.DifficultyEntry[diff]);
                }

                if (cInfo.Family != difficultyInfo.Family)
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}, family: {1}) has different `family` in difficulty {2} mode (Entry: {3}, family: {4}).",
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) has different `trainer_class` in difficulty {1} mode (Entry: {2}).", cInfo.Entry, diff + 1, cInfo.DifficultyEntry[diff]);
                    continue;
                }

                if (cInfo.CreatureType != difficultyInfo.CreatureType)
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}, type: {1}) has different `type` in difficulty {2} mode (Entry: {3}, type: {4}).",
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Non-vehicle Creature (Entry: {0}, VehicleId: {1}) has `VehicleId` set in difficulty {2} mode (Entry: {3}, VehicleId: {4}).",
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}, RegenHealth: {1}) has different `RegenHealth` in difficulty {2} mode (Entry: {3}, RegenHealth: {4}).",
                        cInfo.Entry, cInfo.RegenHealth, diff + 1, cInfo.DifficultyEntry[diff], difficultyInfo.RegenHealth);
                    Log.outWarn(LogFilter.Sql, "Possible FIX: UPDATE `creature_template` SET `RegenHealth`={0} WHERE `entry`={1};", cInfo.RegenHealth, cInfo.DifficultyEntry[diff]);
                }

                uint differenceMask = cInfo.MechanicImmuneMask & (~difficultyInfo.MechanicImmuneMask);
                if (differenceMask != 0)
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}, mechanic_immune_mask: {1}) has weaker immunities in difficulty {2} mode (Entry: {3}, mechanic_immune_mask: {4}).",
                        cInfo.Entry, cInfo.MechanicImmuneMask, diff + 1, cInfo.DifficultyEntry[diff], difficultyInfo.MechanicImmuneMask);
                    Log.outWarn(LogFilter.Sql, "Possible FIX: UPDATE `creature_template` SET `mechanic_immune_mask`=`mechanic_immune_mask`|{0} WHERE `entry`={1};", differenceMask, cInfo.DifficultyEntry[diff]);
                }

                differenceMask = (uint)((cInfo.FlagsExtra ^ difficultyInfo.FlagsExtra) & (~CreatureFlagsExtra.InstanceBind));
                if (differenceMask != 0)
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}, flags_extra: {1}) has different `flags_extra` in difficulty {2} mode (Entry: {3}, flags_extra: {4}).",
                        cInfo.Entry, cInfo.FlagsExtra, diff + 1, cInfo.DifficultyEntry[diff], difficultyInfo.FlagsExtra);
                    Log.outWarn(LogFilter.Sql, "Possible FIX: UPDATE `creature_template` SET `flags_extra`=`flags_extra`^{0} WHERE `entry`={1};", differenceMask, cInfo.DifficultyEntry[diff]);
                }

                if (difficultyInfo.AIName.IsEmpty())
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) lists difficulty {1} mode entry {2} with `AIName` filled in. `AIName` of difficulty 0 mode creature is always used instead.",
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) lists difficulty {1} mode entry {2} with `ScriptName` filled in. `ScriptName` of difficulty 0 mode creature is always used instead.",
                        cInfo.Entry, diff + 1, cInfo.DifficultyEntry[diff]);
                    continue;
                }
            ...
            {
                Log.outWarn(LogFilter.Sql, $"Creature (Entry: {cInfo.Entry}) has `mingold` {cInfo.MinGold} which is greater than `maxgold` {cInfo.MaxGold}, setting `maxgold` to {cInfo.MinGold}.");
                cInfo.MaxGold = cInfo.MinGold;
            }

            if (!CliDB.FactionTemplateStorage.ContainsKey(cInfo.Faction))
            {
                Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) has non-existing faction template ({1}). This can lead to crashes, set to faction 35", cInfo.Entry, cInfo.Faction);
                cInfo.Faction = 35;
            }
            ...
            for (int k = 0; k < SharedConst.MaxCreatureKillCredit; ++k)
            {
                if (cInfo.KillCredit[k] != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) lists non-existing creature entry {1} in `KillCredit{2}`.", cInfo.Entry, cInfo.KillCredit[k], k + 1);
            ...
            if (cInfo.Models.Empty())
                Log.outWarn(LogFilter.Sql, $"Creature (Entry: {cInfo.Entry}) does not have any existing display id in creature_template_model.");

            if (cInfo.UnitClass == 0 || ((1 << ((int)cInfo.UnitClass - 1)) & (int)Class.ClassMaskAllCreatures) == 0)
            {
                Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) has invalid unit_class ({1}) in creature_template. Set to 1 (UNIT_CLASS_WARRIOR).", cInfo.Entry, cInfo.UnitClass);
                cInfo.UnitClass = (uint)Class.Warrior;
            }

            if (cInfo.DmgSchool >= (uint)SpellSchools.Max)
            {
                Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) has invalid spell school value ({1}) in `dmgschool`.", cInfo.Entry, cInfo.DmgSchool);
            ...
            {
                Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) has wrong value ({1}) in speed_walk, set to 1.", cInfo.Entry, cInfo.SpeedWalk);
                cInfo.SpeedWalk = 1.0f;
            }

            if (cInfo.SpeedRun == 0.0f)
            {
                Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) has wrong value ({1}) in speed_run, set to 1.14286.", cInfo.Entry, cInfo.SpeedRun);
                cInfo.SpeedRun = 1.14286f;
            }

            if (cInfo.CreatureType != 0 && !CliDB.CreatureTypeStorage.ContainsKey((uint)cInfo.CreatureType))
            {
                Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) has invalid creature type ({1}) in `type`.", cInfo.Entry, cInfo.CreatureType);
                cInfo.CreatureType = CreatureType.Humanoid;
            }

            if (cInfo.Family != 0 && !CliDB.CreatureFamilyStorage.ContainsKey(cInfo.Family))
            {
                Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) has invalid creature family ({1}) in `family`.", cInfo.Entry, cInfo.Family);
            ...
            {
                Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) has wrong value ({1}) in `HoverHeight`", cInfo.Entry, cInfo.HoverHeight);
            ...
            if (cInfo.VehicleId != 0)
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) has a non-existing VehicleId ({1}). This *WILL* cause the client to freeze!", cInfo.Entry, cInfo.VehicleId);
                    cInfo.VehicleId = 0;
                }
            }
            ...
            for (byte j = 0; j < SharedConst.MaxCreatureSpells; ++j)
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) has non-existing Spell{1} ({2}), set to 0.", cInfo.Entry, j + 1, cInfo.Spells[j]);
                    cInfo.Spells[j] = 0;
                }
            }
            ...
            {
                Log.outWarn(LogFilter.Sql, "Creature (Entry: {0}) has wrong movement generator type ({1}), ignored and set to IDLE.", cInfo.Entry, cInfo.MovementType);
            ...
            {
                Log.outWarn(LogFilter.Sql, "Table `creature_template` lists creature (Id: {0}) with invalid `HealthScalingExpansion` {1}. Ignored and set to 0.", cInfo.Entry, cInfo.HealthScalingExpansion);
                cInfo.HealthScalingExpansion = 0;
            }

            if (cInfo.RequiredExpansion > (int)Expansion.Max)
            {
                Log.outWarn(LogFilter.Sql, "Table `creature_template` lists creature (Entry: {0}) with `RequiredExpansion` {1}. Ignored and set to 0.", cInfo.Entry, cInfo.RequiredExpansion);
            ...
            if (badFlags != 0)
            {
                Log.outWarn(LogFilter.Sql, "Table `creature_template` lists creature (Entry: {0}) with disallowed `flags_extra` {1}, removing incorrect flag.", cInfo.Entry, badFlags);
            ...
            {
                Log.outWarn(LogFilter.Sql, $"Table `creature_template` lists creature (Entry: {cInfo.Entry}) with disallowed `unit_flags` {disallowedUnitFlags}, removing incorrect flag.");
            ...
            {
                Log.outWarn(LogFilter.Sql, $"Table `creature_template` lists creature (Entry: {cInfo.Entry}) with disallowed `unit_flags2` {disallowedUnitFlags2}, removing incorrect flag.");
            ...
            {
                Log.outWarn(LogFilter.Sql, $"Table `creature_template` lists creature (Entry: {cInfo.Entry}) with disallowed `unit_flags2` {disallowedUnitFlags3}, removing incorrect flag.");
                cInfo.UnitFlags3 &= (uint)UnitFlags3.Allowed;
            }

            if (cInfo.DynamicFlags != 0)
            {
                Log.outWarn(LogFilter.Sql, $"Table `creature_template` lists creature (Entry: {cInfo.Entry}) with `dynamicflags` > 0. Ignored and set to 0.");
            ...
            {
                Log.outWarn(LogFilter.Sql, $"Creature (ID: {cInfo.Entry}): Calculated minLevel {cInfo.Minlevel} is not within [1, 255], value has been set to {(cInfo.HealthScalingExpansion == (int)Expansion.LevelCurrent ? SharedConst.MaxLevel : 1)}.");
            ...
            {
                Log.outWarn(LogFilter.Sql, $"Creature (ID: {cInfo.Entry}): Calculated maxLevel {cInfo.Maxlevel} is not within [1, 255], value has been set to {(cInfo.HealthScalingExpansion == (int)Expansion.LevelCurrent ? SharedConst.MaxLevel : 1)}.");
            ...
            if (cInfo.GossipMenuId != 0 && !cInfo.Npcflag.HasAnyFlag((ulong)NPCFlags.Gossip))
                Log.outWarn(LogFilter.Sql, $"Creature (Entry: {cInfo.Entry}) has assigned gossip menu {cInfo.GossipMenuId}, but npcflag does not include UNIT_NPC_FLAG_GOSSIP.");
            else if (cInfo.GossipMenuId == 0 && cInfo.Npcflag.HasAnyFlag((ulong)NPCFlags.Gossip))
                Log.outWarn(LogFilter.Sql, $"Creature (Entry: {cInfo.Entry}) has npcflag UNIT_NPC_FLAG_GOSSIP, but gossip menu is unassigned.");
        }
        void CheckCreatureMovement(string table, ulong id, CreatureMovementData creatureMovement)
        {
            ...
            {
                Log.outWarn(LogFilter.Sql, $"`{table}`.`Ground` wrong value ({creatureMovement.Ground}) for Id {id}, setting to Run.");
                creatureMovement.Ground = CreatureGroundMovementType.Run;
            }

            if (creatureMovement.Flight >= CreatureFlightMovementType.Max)
            {
                Log.outWarn(LogFilter.Sql, $"`{table}`.`Flight` wrong value ({creatureMovement.Flight}) for Id {id}, setting to None.");
                creatureMovement.Flight = CreatureFlightMovementType.None;
            }

            if (creatureMovement.Chase >= CreatureChaseMovementType.Max)
            {
                Log.outWarn(LogFilter.Sql, $"`{table}`.`Chase` wrong value ({creatureMovement.Chase}) for Id {id}, setting to Run.");
                creatureMovement.Chase = CreatureChaseMovementType.Run;
            }

            if (creatureMovement.Random >= CreatureRandomMovementType.Max)
            {
                Log.outWarn(LogFilter.Sql, $"`{table}`.`Random` wrong value ({creatureMovement.Random}) for Id {id}, setting to Walk.");
            ...
            do
            {
                ...
                switch ((CreatureLinkedRespawnType)linkType)
                {
                    case CreatureLinkedRespawnType.CreatureToCreature:
                    {
                        ...
                        if (slave == null)
                        {
                            Log.outWarn(LogFilter.Sql, "Couldn't get creature data for GUIDLow {0}", guidLow);
                            error = true;
                            break;
                        }
                        ...
                        if (master == null)
                        {
                            Log.outWarn(LogFilter.Sql, "Couldn't get creature data for GUIDLow {0}", linkedGuidLow);
                            error = true;
                            break;
                        }
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Creature '{0}' linking to '{1}' on an unpermitted map.", guidLow, linkedGuidLow);
                            error = true;
                            break;
                        }
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "LinkedRespawn: Creature '{0}' linking to '{1}' with not corresponding spawnMask", guidLow, linkedGuidLow);
                            error = true;
                            break;
                        }
                    ...
                    case CreatureLinkedRespawnType.CreatureToGO:
                    {
                        ...
                        if (master == null)
                        {
                            Log.outWarn(LogFilter.Sql, "Couldn't get gameobject data for GUIDLow {0}", linkedGuidLow);
                            error = true;
                            break;
                        }
                    ...
                    case CreatureLinkedRespawnType.GOToGO:
                    {
                        ...
                        if (slave == null)
                        {
                            Log.outWarn(LogFilter.Sql, "Couldn't get gameobject data for GUIDLow {0}", guidLow);
                            error = true;
                            break;
                        }
        ...
        public void LoadNPCText()
        {
            ...
            do
            {
                ...
                if (textID == 0)
                {
                    Log.outWarn(LogFilter.Sql, "Table `npc_text` has record wit reserved id 0, ignore.");
                    continue;
                }
                ...
                for (int i = 0; i < SharedConst.MaxNpcTextOptions; i++)
                {
                    if (npcText.Data[i].BroadcastTextID != 0)
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "NPCText (Id: {0}) has a non-existing BroadcastText (ID: {1}, Index: {2})", textID, npcText.Data[i].BroadcastTextID, i);
                ...
                for (byte i = 0; i < SharedConst.MaxNpcTextOptions; i++)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "NPCText (ID: {0}) has a probability (Index: {1}) set, but no BroadcastTextID to go with it", textID, i);
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"NPCText (ID: {textID}) has a probability sum 0, no text can be selected from it, skipped.");
                    continue;
                }
        ...
        public void LoadTrainers()
        {
            ...
            if (!trainerSpellsResult.IsEmpty())
            {
                do
                {
                    ...
                    if (spellInfo == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `trainer_spell` references non-existing spell (SpellId: {spell.SpellId}) for TrainerId {trainerId}, ignoring");
                        continue;
                    }

                    if (spell.ReqSkillLine != 0 && !CliDB.SkillLineStorage.ContainsKey(spell.ReqSkillLine))
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `trainer_spell` references non-existing skill (ReqSkillLine: {spell.ReqSkillLine}) for TrainerId {trainerId} and SpellId {spell.SpellId}, ignoring");
                        continue;
                    }
                    ...
                    for (var i = 0; i < spell.ReqAbility.Count; ++i)
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, $"Table `trainer_spell` references non-existing spell (ReqAbility {i + 1}: {requiredSpell}) for TrainerId {trainerId} and SpellId {spell.SpellId}, ignoring");
                            allReqValid = false;
                        }
                    }
            ...
            {
                Log.outWarn(LogFilter.Sql, $"Table `trainer_spell` references non-existing trainer (TrainerId: {unusedSpells.Key}) for SpellId {unusedSpells.Value.SpellId}, ignoring");
            ...
            if (!trainerLocalesResult.IsEmpty())
            {
                do
                {
                    ...
                    if (trainer != null)
                        ...
                        Log.outWarn(LogFilter.Sql, $"Table `trainer_locale` references non-existing trainer (TrainerId: {trainerId}) for locale {localeName}, ignoring");
        ...
        public void LoadCreatureTrainers()
        {
            ...
            if (!result.IsEmpty())
            {
                do
                {
                    ...
                    if (GetCreatureTemplate(creatureId) == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `creature_trainer` references non-existing creature template (CreatureId: {creatureId}), ignoring");
                        continue;
                    }

                    if (GetTrainer(trainerId) == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `creature_trainer` references non-existing trainer (TrainerId: {trainerId}) for CreatureId {creatureId} MenuId {gossipMenuId} OptionIndex {gossipOptionIndex}, ignoring");
                        continue;
                    }
                    ...
                    if (gossipMenuId != 0 || gossipOptionIndex != 0)
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, $"Table `creature_trainer` references non-existing gossip menu option (MenuId {gossipMenuId} OptionIndex {gossipOptionIndex}) for CreatureId {creatureId} and TrainerId {trainerId}, ignoring");
                            continue;
                        }
                    }
        ...
        public void LoadCreatures()
        {
            ...
            do
            {
                ...
                if (cInfo == null)
                {
                    Log.outWarn(LogFilter.Sql, "Table `creature` has creature (GUID: {0}) with non existing creature entry {1}, skipped.", guid, entry);
                    continue;
                }
                ...
                if (mapEntry == null)
                {
                    Log.outWarn(LogFilter.Sql, "Table `creature` have creature (GUID: {0}) that spawned at not existed map (Id: {1}), skipped.", guid, data.MapId);
                    continue;
                }

                if (data.SpawnDifficulties.Empty())
                {
                    Log.outWarn(LogFilter.Sql, $"Table `creature` has creature (GUID: {guid}) that is not spawned in any difficulty, skipped.");
                    continue;
                }
                ...
                for (uint diff = 0; diff < SharedConst.MaxCreatureDifficulties && ok; ++diff)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Table `creature` have creature (GUID: {0}) that listed as difficulty {1} template (entry: {2}) in `creaturetemplate`, skipped.", guid, diff + 1, data.Id);
                        ok = false;
                    }
                }
                if (!ok)
                ...
                if (data.equipmentId != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Table `creature` have creature (Entry: {0}) with equipmentid {1} not found in table `creatureequiptemplate`, set to no equipment.", data.Id, data.equipmentId);
                        data.equipmentId = 0;
                    }
                }
                ...
                if (cInfo.FlagsExtra.HasAnyFlag(CreatureFlagsExtra.InstanceBind))
                {
                    if (!mapEntry.IsDungeon())
                        Log.outWarn(LogFilter.Sql, "Table `creature` have creature (GUID: {0} Entry: {1}) with `creature_template`.`flagsextra` including CREATUREFLAGEXTRAINSTANCEBIND " +
                            "but creature are not in instance.", guid, data.Id);
                }

                if (data.WanderDistance < 0.0f)
                {
                    Log.outWarn(LogFilter.Sql, "Table `creature` have creature (GUID: {0} Entry: {1}) with `wander_distance`< 0, set to 0.", guid, data.Id);
                ...
                else if (data.movementType == (byte)MovementGeneratorType.Random)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Table `creature` have creature (GUID: {0} Entry: {1}) with `MovementType`=1 (random movement) but with `wander_distance`=0, replace by idle movement type (0).", guid, data.Id);
                ...
                else if (data.movementType == (byte)MovementGeneratorType.Idle)
                {
                    if (data.WanderDistance != 0.0f)
                    {
                        Log.outWarn(LogFilter.Sql, "Table `creature` have creature (GUID: {0} Entry: {1}) with `MovementType`=0 (idle) have `wander_distance`<>0, set to 0.", guid, data.Id);
                        data.WanderDistance = 0.0f;
                    }
                }
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Table `creature` have creature (GUID: {0} Entry: {1}) has unknown `phaseUseFlags` set, removed unknown value.", guid, data.Id);
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Table `creature` have creature (GUID: {0} Entry: {1}) has both `phaseUseFlags` PHASE_USE_FLAGS_ALWAYS_VISIBLE and PHASE_USE_FLAGS_INVERSE," +
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Table `creature` have creature (GUID: {0} Entry: {1}) with both `phaseid` and `phasegroup` set, `phasegroup` set to 0", guid, data.Id);
                    data.PhaseGroup = 0;
                }
                ...
                if (data.PhaseId != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Table `creature` have creature (GUID: {0} Entry: {1}) with `phaseid` {2} does not exist, set to 0", guid, data.Id, data.PhaseId);
                        data.PhaseId = 0;
                    }
                }
                ...
                if (data.PhaseGroup != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Table `creature` have creature (GUID: {0} Entry: {1}) with `phasegroup` {2} does not exist, set to 0", guid, data.Id, data.PhaseGroup);
                        data.PhaseGroup = 0;
                    }
                }
                ...
                if (data.terrainSwapMap != -1)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Table `creature` have creature (GUID: {0} Entry: {1}) with `terrainSwapMap` {2} does not exist, set to -1", guid, data.Id, data.terrainSwapMap);
                        data.terrainSwapMap = -1;
                    }
                    else if (terrainSwapEntry.ParentMapID != data.MapId)
                    {
                        Log.outWarn(LogFilter.Sql, "Table `creature` have creature (GUID: {0} Entry: {1}) with `terrainSwapMap` {2} which cannot be used on spawn map, set to -1", guid, data.Id, data.terrainSwapMap);
                        data.terrainSwapMap = -1;
                    }
                }
        ...
        public bool SetCreatureLinkedRespawn(ulong guidLow, ulong linkedGuidLow)
        {
            ...
            if (slave == null)
            {
                Log.outWarn(LogFilter.Sql, "Creature '{0}' linking to non-existent creature '{1}'.", guidLow, linkedGuidLow);
                return false;
            }
        ...
        public CreatureModelInfo GetCreatureModelRandomGender(ref CreatureModel model, CreatureTemplate creatureTemplate)
        {
            ...
            if (modelInfo.DisplayIdOtherGender != 0 && RandomHelper.URand(0, 1) == 0)
            {
                ...
                if (minfotmp == null)
                    Log.outWarn(LogFilter.Sql, $"Model (Entry: {model.CreatureDisplayID}) has modelidothergender {modelInfo.DisplayIdOtherGender} not found in table `creaturemodelinfo`. ");
                else
                {
                    // DisplayID changed
                ...
                do
                {
                    ...
                    switch (got.type)
                    {
                        ...
                        case GameObjectTypes.SpellFocus:               //8
                            if (got.SpellFocus.spellFocusType != 0)
                            {
                                if (!CliDB.SpellFocusObjectStorage.ContainsKey(got.SpellFocus.spellFocusType))
                                    Log.outWarn(LogFilter.Sql, "GameObject (Entry: {0} GoType: {1}) have data0={2} but SpellFocus (Id: {3}) not exist.",
                        ...
                        case GameObjectTypes.Goober:                    //10
                            ...
                            if (got.Goober.pageID != 0)                  // pageId
                            {
                                if (GetPageText(got.Goober.pageID) == null)
                                    Log.outWarn(LogFilter.Sql, "GameObject (Entry: {0} GoType: {1}) have data7={2} but PageText (Entry {3}) not exist.", entry, got.type, got.Goober.pageID, got.Goober.pageID);
                            }
                        ...
                        case GameObjectTypes.MapObjTransport:              //15
                        {
                            if (got.MoTransport.taxiPathID != 0)
                            {
                                if (got.MoTransport.taxiPathID >= CliDB.TaxiPathNodesByPath.Count || CliDB.TaxiPathNodesByPath[got.MoTransport.taxiPathID].Empty())
                                    Log.outWarn(LogFilter.Sql, "GameObject (Entry: {0} GoType: {1}) have data0={2} but TaxiPath (Id: {3}) not exist.",
                        ...
                        case GameObjectTypes.BarberChair:              //32
                            ...
                            {
                                Log.outWarn(LogFilter.Sql, "GameObject (Entry: {0} GoType: {1}) have data2 = {2} but AnimKit.dbc (Id: {3}) not exist, set to 0.",
        ...
        public void LoadGameObjectTemplateAddons()
        {
            ...
            do
            {
                ...
                if (got == null)
                {
                    Log.outWarn(LogFilter.Sql, $"GameObject template (Entry: {entry}) does not exist but has a record in `gameobject_template_addon`");
                    continue;
                }
                ...
                for (int i = 0; i < gameObjectAddon.ArtKits.Length; ++i)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, $"GameObject (Entry: {entry}) has invalid `artkit{i}` ({artKitID}) defined, set to zero instead.");
                        continue;
                    }
                ...
                if (gameObjectAddon.Faction != 0 && !CliDB.FactionTemplateStorage.ContainsKey(gameObjectAddon.Faction))
                    Log.outWarn(LogFilter.Sql, $"GameObject (Entry: {entry}) has invalid faction ({gameObjectAddon.Faction}) defined in `gameobject_template_addon`.");
                ...
                if (gameObjectAddon.Maxgold > 0)
                {
                    switch (got.type)
                    {
                        ...
                        case GameObjectTypes.FishingHole:
                            break;
                        default:
                            Log.outWarn(LogFilter.Sql, $"GameObject (Entry {entry} GoType: {got.type}) cannot be looted but has maxgold set in `gameobject_template_addon`.");
                            break;
                    }
                }
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"GameObject (Entry: {entry}) has invalid WorldEffectID ({gameObjectAddon.WorldEffectID}) defined in `gameobject_template_addon`, set to 0.");
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"GameObject (Entry: {entry}) has invalid AIAnimKitID ({gameObjectAddon.AIAnimKitID}) defined in `gameobject_template_addon`, set to 0.");
        ...
        public void LoadGameObjectOverrides()
        {
            ...
            do
            {
                ...
                if (goData == null)
                {
                    Log.outWarn(LogFilter.Sql, $"GameObject (SpawnId: {spawnId}) does not exist but has a record in `gameobject_overrides`");
                    continue;
                }
                ...
                if (gameObjectOverride.Faction != 0 && !CliDB.FactionTemplateStorage.ContainsKey(gameObjectOverride.Faction))
                    Log.outWarn(LogFilter.Sql, $"GameObject (SpawnId: {spawnId}) has invalid faction ({gameObjectOverride.Faction}) defined in `gameobject_overrides`.");
        ...
        public void LoadGameObjects()
        {
            ...
            do
            {
                ...
                if (gInfo == null)
                {
                    Log.outWarn(LogFilter.Sql, "Table `gameobject` has gameobject (GUID: {0}) with non existing gameobject entry {1}, skipped.", guid, entry);
                    continue;
                }
                ...
                if (gInfo.displayId == 0)
                {
                    switch (gInfo.type)
                    {
                        ...
                        case GameObjectTypes.SpellFocus:
                            break;
                        default:
                            Log.outWarn(LogFilter.Sql, "Gameobject (GUID: {0} Entry {1} GoType: {2}) doesn't have a displayId ({3}), not loaded.", guid, entry, gInfo.type, gInfo.displayId);
                            break;
                    }
                }
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Gameobject (GUID: {0} Entry {1} GoType: {2}) has an invalid displayId ({3}), not loaded.", guid, entry, gInfo.type, gInfo.displayId);
                    continue;
                }
                ...
                if (mapEntry == null)
                {
                    Log.outWarn(LogFilter.Sql, "Table `gameobject` has gameobject (GUID: {0} Entry: {1}) spawned on a non-existed map (Id: {2}), skip", guid, data.Id, data.MapId);
                    continue;
                }

                if (data.spawntimesecs == 0 && gInfo.IsDespawnAtAction())
                {
                    Log.outWarn(LogFilter.Sql, "Table `gameobject` has gameobject (GUID: {0} Entry: {1}) with `spawntimesecs` (0) value, but the gameobejct is marked as despawnable at action.", guid, data.Id);
                }
                ...
                if (gostate >= (uint)GameObjectState.Max)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Table `gameobject` has gameobject (GUID: {0} Entry: {1}) with invalid `state` ({2}) value, skip", guid, data.Id, gostate);
                        continue;
                    }
                }
                ...
                if (Convert.ToBoolean(data.PhaseUseFlags & ~PhaseUseFlagsValues.All))
                {
                    Log.outWarn(LogFilter.Sql, "Table `gameobject` have gameobject (GUID: {0} Entry: {1}) has unknown `phaseUseFlags` set, removed unknown value.", guid, data.Id);
                    data.PhaseUseFlags &= PhaseUseFlagsValues.All;
                }

                if (data.PhaseUseFlags.HasAnyFlag(PhaseUseFlagsValues.AlwaysVisible) && data.PhaseUseFlags.HasAnyFlag(PhaseUseFlagsValues.Inverse))
                {
                    Log.outWarn(LogFilter.Sql, "Table `gameobject` have gameobject (GUID: {0} Entry: {1}) has both `phaseUseFlags` PHASE_USE_FLAGS_ALWAYS_VISIBLE and PHASE_USE_FLAGS_INVERSE," +
                        " removing PHASE_USE_FLAGS_INVERSE.", guid, data.Id);
                    data.PhaseUseFlags &= ~PhaseUseFlagsValues.Inverse;
                }
                ...
                if (data.PhaseGroup != 0 && data.PhaseId != 0)
                {
                    Log.outWarn(LogFilter.Sql, "Table `gameobject` have gameobject (GUID: {0} Entry: {1}) with both `phaseid` and `phasegroup` set, `phasegroup` set to 0", guid, data.Id);
                    data.PhaseGroup = 0;
                }
                ...
                if (data.PhaseId != 0)
                {
                    if (!CliDB.PhaseStorage.ContainsKey(data.PhaseId))
                    {
                        Log.outWarn(LogFilter.Sql, "Table `gameobject` have gameobject (GUID: {0} Entry: {1}) with `phaseid` {2} does not exist, set to 0", guid, data.Id, data.PhaseId);
                        data.PhaseId = 0;
                    }
                }
                ...
                if (data.PhaseGroup != 0)
                {
                    if (Global.DB2Mgr.GetPhasesForGroup(data.PhaseGroup).Empty())
                    {
                        Log.outWarn(LogFilter.Sql, "Table `gameobject` have gameobject (GUID: {0} Entry: {1}) with `phaseGroup` {2} does not exist, set to 0", guid, data.Id, data.PhaseGroup);
                        data.PhaseGroup = 0;
                    }
                }
                ...
                if (data.terrainSwapMap != -1)
                {
                    ...
                    if (terrainSwapEntry == null)
                    {
                        Log.outWarn(LogFilter.Sql, "Table `gameobject` have gameobject (GUID: {0} Entry: {1}) with `terrainSwapMap` {2} does not exist, set to -1", guid, data.Id, data.terrainSwapMap);
                        data.terrainSwapMap = -1;
                    }
                    else if (terrainSwapEntry.ParentMapID != data.MapId)
                    {
                        Log.outWarn(LogFilter.Sql, "Table `gameobject` have gameobject (GUID: {0} Entry: {1}) with `terrainSwapMap` {2} which cannot be used on spawn map, set to -1", guid, data.Id, data.terrainSwapMap);
                        data.terrainSwapMap = -1;
                    }
                }
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Table `gameobject` has gameobject (GUID: {0} Entry: {1}) with invalid rotationX ({2}) value, skip", guid, data.Id, data.rotation.X);
                    continue;
                }

                if (data.rotation.Y < -1.0f || data.rotation.Y > 1.0f)
                {
                    Log.outWarn(LogFilter.Sql, "Table `gameobject` has gameobject (GUID: {0} Entry: {1}) with invalid rotationY ({2}) value, skip", guid, data.Id, data.rotation.Y);
                    continue;
                }

                if (data.rotation.Z < -1.0f || data.rotation.Z > 1.0f)
                {
                    Log.outWarn(LogFilter.Sql, "Table `gameobject` has gameobject (GUID: {0} Entry: {1}) with invalid rotationZ ({2}) value, skip", guid, data.Id, data.rotation.Z);
                    continue;
                }

                if (data.rotation.W < -1.0f || data.rotation.W > 1.0f)
                {
                    Log.outWarn(LogFilter.Sql, "Table `gameobject` has gameobject (GUID: {0} Entry: {1}) with invalid rotationW ({2}) value, skip", guid, data.Id, data.rotation.W);
                    continue;
                }

                if (!GridDefines.IsValidMapCoord(data.MapId, data.SpawnPoint))
                {
                    Log.outWarn(LogFilter.Sql, "Table `gameobject` has gameobject (GUID: {0} Entry: {1}) with invalid coordinates, skip", guid, data.Id);
                    continue;
                }

                if (!(Math.Abs(Quaternion.Dot(data.rotation, data.rotation) - 1) < 1e-5))
                {
                    Log.outWarn(LogFilter.Sql, $"Table `gameobject` has gameobject (GUID: {guid} Entry: {data.Id}) with invalid rotation quaternion (non-unit), defaulting to orientation on Z axis only");
        ...
        public void LoadGameObjectAddons()
        {
            ...
            do
            {
                ...
                if (goData == null)
                {
                    Log.outWarn(LogFilter.Sql, $"GameObject (GUID: {guid}) does not exist but has a record in `gameobject_addon`");
                    continue;
                }
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"GameObject (GUID: {guid}) has invalid InvisibilityType in `gameobject_addon`, disabled invisibility");
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"GameObject (GUID: {guid}) has InvisibilityType set but has no InvisibilityValue in `gameobject_addon`, set to 1");
                    gameObjectAddon.invisibilityValue = 1;
                }

                if (!(Math.Abs(Quaternion.Dot(gameObjectAddon.ParentRotation, gameObjectAddon.ParentRotation) - 1) < 1e-5))
                {
                    Log.outWarn(LogFilter.Sql, $"GameObject (GUID: {guid}) has invalid parent rotation in `gameobject_addon`, set to default");
                    gameObjectAddon.ParentRotation = Quaternion.Identity;
                }

                if (gameObjectAddon.WorldEffectID != 0 && !CliDB.WorldEffectStorage.ContainsKey(gameObjectAddon.WorldEffectID))
                {
                    Log.outWarn(LogFilter.Sql, $"GameObject (GUID: {guid}) has invalid WorldEffectID ({gameObjectAddon.WorldEffectID}) in `gameobject_addon`, set to 0.");
                    gameObjectAddon.WorldEffectID = 0;
                }

                if (gameObjectAddon.AIAnimKitID != 0 && !CliDB.AnimKitStorage.ContainsKey(gameObjectAddon.AIAnimKitID))
                {
                    Log.outWarn(LogFilter.Sql, $"GameObject (GUID: {guid}) has invalid AIAnimKitID ({gameObjectAddon.AIAnimKitID}) in `gameobject_addon`, set to 0.");
                    gameObjectAddon.AIAnimKitID = 0;
                }
        ...
        public void LoadGameObjectQuestItems()
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Table `gameobject_questitem` has data for nonexistent gameobject (entry: {0}, idx: {1}), skipped", entry, idx);
                    continue;
                }

                if (!CliDB.ItemStorage.ContainsKey(item))
                {
                    Log.outWarn(LogFilter.Sql, "Table `gameobject_questitem` has nonexistent item (ID: {0}) in gameobject (entry: {1}, idx: {2}), skipped", item, entry, idx);
                    continue;
                }
        ...
        void CheckGOLockId(GameObjectTemplate goInfo, uint dataN, uint N)
        {
            ...
            Log.outWarn(LogFilter.Sql, "Gameobject (Entry: {0} GoType: {1}) have data{2}={3} but lock (Id: {4}) not found.", goInfo.entry, goInfo.type, N, goInfo.Door.open, goInfo.Door.open);
        }
        void CheckGOLinkedTrapId(GameObjectTemplate goInfo, uint dataN, uint N)
        {
            ...
            if (trapInfo != null)
            {
                if (trapInfo.type != GameObjectTypes.Trap)
                    Log.outWarn(LogFilter.Sql, "Gameobject (Entry: {0} GoType: {1}) have data{2}={3} but GO (Entry {4}) have not GAMEOBJECT_TYPE_TRAP type.", goInfo.entry, goInfo.type, N, dataN, dataN);
            }
        }
        void CheckGOSpellId(GameObjectTemplate goInfo, uint dataN, uint N)
        {
            ...
            Log.outWarn(LogFilter.Sql, "Gameobject (Entry: {0} GoType: {1}) have data{2}={3} but Spell (Entry {4}) not exist.", goInfo.entry, goInfo.type, N, dataN, dataN);
        }
        void CheckAndFixGOChairHeightId(GameObjectTemplate goInfo, ref uint dataN, uint N)
        {
            ...
            Log.outWarn(LogFilter.Sql, "Gameobject (Entry: {0} GoType: {1}) have data{2}={3} but correct chair height in range 0..{4}.", goInfo.entry, goInfo.type, N, dataN, UnitStandStateType.SitHighChair - UnitStandStateType.SitLowChair);
        ...
        void CheckGONoDamageImmuneId(GameObjectTemplate goTemplate, uint dataN, uint N)
        {
            ...
            Log.outWarn(LogFilter.Sql, "Gameobject (Entry: {0} GoType: {1}) have data{2}={3} but expected boolean (0/1) noDamageImmune field value.", goTemplate.entry, goTemplate.type, N, dataN);
        }
        void CheckGOConsumable(GameObjectTemplate goInfo, uint dataN, uint N)
        {
            ...
            Log.outWarn(LogFilter.Sql, "Gameobject (Entry: {0} GoType: {1}) have data{2}={3} but expected boolean (0/1) consumable field value.",
        ...
        List<Difficulty> ParseSpawnDifficulties(string difficultyString, string table, ulong spawnId, uint mapId, List<Difficulty> mapDifficulties)
        {
            ...
            foreach (string token in tokens)
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"Table `{table}` has {table} (GUID: {spawnId}) with non invalid difficulty id {difficultyId}, skipped.");
                    continue;
                }

                if (!isTransportMap && !mapDifficulties.Contains(difficultyId))
                {
                    Log.outWarn(LogFilter.Sql, $"Table `{table}` has {table} (GUID: {spawnId}) has unsupported difficulty {difficultyId} for map (Id: {mapId}).");
                    continue;
                }
        ...
        public void LoadItemTemplateAddon()
        {
            ...
            if (!result.IsEmpty())
            {
                do
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Item {0} specified in `itemtemplateaddon` does not exist, skipped.", itemId);
                        continue;
                    }
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Minimum money loot specified in `itemtemplateaddon` for item {0} was greater than maximum amount, swapping.", itemId);
        ...
        public void LoadItemScriptNames()
        {
            ...
            if (!result.IsEmpty())
            {
                do
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Item {0} specified in `item_script_names` does not exist, skipped.", itemId);
                        continue;
                    }
        ...
        public bool IsVendorItemValid(uint vendorentry, VendorItem vItem, Player player = null, List<uint> skipvendors = null, ulong ORnpcflag = 0)
        {
            ...
            if (cInfo == null)
            {
                if (player != null)
                    ...
                    Log.outWarn(LogFilter.Sql, "Table `(gameevent)npcvendor` have data for not existed creature template (Entry: {0}), ignore", vendorentry);
                return false;
            }
            ...
            if (!Convert.ToBoolean(((ulong)cInfo.Npcflag | ORnpcflag) & (ulong)NPCFlags.Vendor))
            {
                if (skipvendors == null || skipvendors.Count == 0)
                {
                    if (player != null)
                        ...
                        Log.outWarn(LogFilter.Sql, "Table `(gameevent)npcvendor` have data for not creature template (Entry: {0}) without vendor flag, ignore", vendorentry);
                ...
                (vItem.Type == ItemVendorType.Currency && CliDB.CurrencyTypesStorage.LookupByKey(vItem.item) == null))
            {
                if (player != null)
                    ...
                    Log.outWarn(LogFilter.Sql, "Table `(gameevent)npcvendor` for Vendor (Entry: {0}) have in item list non-existed item ({1}, type {2}), ignore", vendorentry, vItem.item, vItem.Type);
                return false;
            }

            if (vItem.PlayerConditionId != 0 && !CliDB.PlayerConditionStorage.ContainsKey(vItem.PlayerConditionId))
            {
                Log.outWarn(LogFilter.Sql, "Table `(game_event_)npc_vendor` has Item (Entry: {0}) with invalid PlayerConditionId ({1}) for vendor ({2}), ignore", vItem.item, vItem.PlayerConditionId, vendorentry);
                return false;
            }
            ...
            if (vItem.ExtendedCost != 0 && !CliDB.ItemExtendedCostStorage.ContainsKey(vItem.ExtendedCost))
            {
                if (player != null)
                    ...
                    Log.outWarn(LogFilter.Sql, "Table `(gameevent)npcvendor` have Item (Entry: {0}) with wrong ExtendedCost ({1}) for vendor ({2}), ignore", vItem.item, vItem.ExtendedCost, vendorentry);
                return false;
            }
            ...
            if (vItem.Type == ItemVendorType.Item) // not applicable to currencies
            {
                if (vItem.maxcount > 0 && vItem.incrtime == 0)
                {
                    if (player != null)
                        ...
                        Log.outWarn(LogFilter.Sql, "Table `(gameevent)npcvendor` has `maxcount` ({0}) for item {1} of vendor (Entry: {2}) but `incrtime`=0, ignore", vItem.maxcount, vItem.item, vendorentry);
                    return false;
                }
                else if (vItem.maxcount == 0 && vItem.incrtime > 0)
                {
                    if (player != null)
                        ...
                        Log.outWarn(LogFilter.Sql, "Table `(gameevent)npcvendor` has `maxcount`=0 for item {0} of vendor (Entry: {0}) but `incrtime`<>0, ignore", vItem.item, vendorentry);
                    return false;
                }
                ...
                foreach (uint bonusList in vItem.BonusListIDs)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Table `(game_event_)npc_vendor` have Item (Entry: {0}) with invalid bonus {1} for vendor ({2}), ignore", vItem.item, bonusList, vendorentry);
                        return false;
                    }
                }
            }
            ...
            if (vItems.FindItemCostPair(vItem.item, vItem.ExtendedCost, vItem.Type) != null)
            {
                if (player != null)
                    ...
                    Log.outWarn(LogFilter.Sql, "Table `npcvendor` has duplicate items {0} (with extended cost {1}, type {2}) for vendor (Entry: {3}), ignoring", vItem.item, vItem.ExtendedCost, vItem.Type, vendorentry);
                return false;
            }

            if (vItem.Type == ItemVendorType.Currency && vItem.maxcount == 0)
            {
                Log.outWarn(LogFilter.Sql, "Table `(game_event_)npc_vendor` have Item (Entry: {0}, type: {1}) with missing maxcount for vendor ({2}), ignore", vItem.item, vItem.Type, vendorentry);
                return false;
            }
        ...
        public void LoadInstanceTemplate()
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, "ObjectMgr.LoadInstanceTemplate: bad mapid {0} for template!", mapID);
                    continue;
                }
        ...
        public void LoadGameTele()
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Wrong position for id {0} (name: {1}) in `game_tele` table, ignoring.", id, gt.name);
                    continue;
                }
        ...
        public void LoadAreaTriggerTeleports()
        {
            ...
            do
            {
                ...
                if (portLoc == null)
                {
                    Log.outWarn(LogFilter.Sql, "Area Trigger (ID: {0}) has a non-existing Port Loc (ID: {1}) in WorldSafeLocs.dbc, skipped", Trigger_ID, PortLocID);
                    continue;
                }
                ...
                if (atEntry == null)
                {
                    Log.outWarn(LogFilter.Sql, "Area trigger (ID: {0}) does not exist in `AreaTrigger.dbc`.", Trigger_ID);
                    continue;
                }
        ...
        public void LoadAccessRequirements()
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Map {0} referenced in `access_requirement` does not exist, skipped.", mapid);
                    continue;
                }
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Map {0} referenced in `access_requirement` does not have difficulty {1}, skipped", mapid, difficulty);
                    continue;
                }
                ...
                if (ar.item != 0)
                {
                    ...
                    if (pProto == null)
                    {
                        Log.outWarn(LogFilter.Sql, "Key item {0} does not exist for map {1} difficulty {2}, removing key requirement.", ar.item, mapid, difficulty);
                        ar.item = 0;
                    }
                }
                ...
                if (ar.item2 != 0)
                {
                    ...
                    if (pProto == null)
                    {
                        Log.outWarn(LogFilter.Sql, "Second item {0} does not exist for map {1} difficulty {2}, removing key requirement.", ar.item2, mapid, difficulty);
                        ar.item2 = 0;
                    }
                }
                ...
                if (ar.quest_A != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Required Alliance Quest {0} not exist for map {1} difficulty {2}, remove quest done requirement.", ar.quest_A, mapid, difficulty);
                        ar.quest_A = 0;
                    }
                }
                ...
                if (ar.quest_H != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Required Horde Quest {0} not exist for map {1} difficulty {2}, remove quest done requirement.", ar.quest_H, mapid, difficulty);
                        ar.quest_H = 0;
                    }
                }
                ...
                if (ar.achievement != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Required Achievement {0} not exist for map {1} difficulty {2}, remove quest done requirement.", ar.achievement, mapid, difficulty);
                        ar.achievement = 0;
                    }
                }
        ...
        public void LoadInstanceEncounters()
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Table `instance_encounters` has an invalid encounter id {0}, skipped!", entry);
                    continue;
                }

                if (lastEncounterDungeon != 0 && Global.LFGMgr.GetLFGDungeonEntry(lastEncounterDungeon) == 0)
                {
                    Log.outWarn(LogFilter.Sql, "Table `instance_encounters` has an encounter {0} ({1}) marked as final for invalid dungeon id {2}, skipped!",
                ...
                if (lastEncounterDungeon != 0)
                {
                    if (pair != null)
                    {
                        Log.outWarn(LogFilter.Sql, "Table `instance_encounters` specified encounter {0} ({1}) as last encounter but {2} ({3}) is already marked as one, skipped!",
                ...
                switch (creditType)
                {
                    case EncounterCreditType.KillCreature:
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `instance_encounters` has an invalid creature (entry {0}) linked to the encounter {1} ({2}), skipped!",
                    ...
                    case EncounterCreditType.CastSpell:
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `instance_encounters` has an invalid spell (entry {0}) linked to the encounter {1} ({2}), skipped!",
                                creditEntry, entry, dungeonEncounter.Name[Global.WorldMgr.GetDefaultDbcLocale()]);
                            continue;
                        }
                        break;
                    default:
                        Log.outWarn(LogFilter.Sql, "Table `instance_encounters` has an invalid credit type ({0}) for encounter {1} ({2}), skipped!",
        ...
        public void LoadSpawnGroupTemplates()
        {
            ...
            if (!result.IsEmpty())
            {
                do
                {
                    ...
                    if (flags.HasAnyFlag(~SpawnGroupFlags.All))
                    {
                        ...
                        Log.outWarn(LogFilter.Sql, $"Invalid spawn group flag {flags} on group ID {groupId} ({group.name}), reduced to valid flag {group.flags}.");
                    }
                    if (flags.HasAnyFlag(SpawnGroupFlags.System) && flags.HasAnyFlag(SpawnGroupFlags.ManualSpawn))
                    {
                        flags &= ~SpawnGroupFlags.ManualSpawn;
                        Log.outWarn(LogFilter.Sql, $"System spawn group {groupId} ({group.name}) has invalid manual spawn flag. Ignored.");
                    }
                    group.flags = flags;
            ...
            {
                Log.outWarn(LogFilter.Sql, "Default spawn group (index 0) is missing from DB! Manually inserted.");
            ...
            if (!_spawnGroupDataStorage.ContainsKey(1))
            {
                ...
                Log.outWarn(LogFilter.Sql, "Default legacy spawn group (index 1) is missing from DB! Manually inserted.");
                SpawnGroupTemplateData data = new();
                data.groupId = 1;
        ...
        public void LoadSpawnGroups()
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"Spawn data with invalid type {spawnType} listed for spawn group {groupId}. Skipped.");
                    continue;
                }
                ...
                if (data == null)
                {
                    Log.outWarn(LogFilter.Sql, $"Spawn data with ID ({spawnType},{spawnId}) not found, but is listed as a member of spawn group {groupId}!");
                    continue;
                }
                else if (data.spawnGroupData.groupId != 0)
                {
                    Log.outWarn(LogFilter.Sql, $"Spawn with ID ({spawnType},{spawnId}) is listed as a member of spawn group {groupId}, but is already a member of spawn group {data.spawnGroupData.groupId}. Skipping.");
                    continue;
                }
                var groupTemplate = _spawnGroupDataStorage.LookupByKey(groupId);
                if (groupTemplate == null)
                {
                    Log.outWarn(LogFilter.Sql, $"Spawn group {groupId} assigned to spawn ID ({spawnType},{spawnId}), but group is found!");
                    continue;
                }
                else
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, $"Spawn group {groupId} has map ID {groupTemplate.mapId}, but spawn ({spawnType},{spawnId}) has map id {data.MapId} - spawn NOT added to group!");
                        continue;
                    }
        ...
        public void LoadInstanceSpawnGroups()
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"Invalid spawn group {spawnGroupId} specified for instance {instanceMapId}. Skipped.");
                    continue;
                }
                ...
                if ((states & ~ALL_STATES) != 0)
                {
                    ...
                    Log.outWarn(LogFilter.Sql, $"Instance spawn group ({instanceMapId},{spawnGroupId}) had invalid boss state mask {states} - truncated to {info.BossStates}.");
                }
                else
                ...
                if ((flags & ~InstanceSpawnGroupFlags.All) != 0)
                {
                    ...
                    Log.outWarn(LogFilter.Sql, $"Instance spawn group ({instanceMapId},{spawnGroupId}) had invalid flags {flags} - truncated to {info.Flags}.");
                }
                else
                    info.Flags = flags;
                ...
                if (flags.HasFlag(InstanceSpawnGroupFlags.AllianceOnly) && flags.HasFlag(InstanceSpawnGroupFlags.HordeOnly))
                {
                    ...
                    Log.outWarn(LogFilter.Sql, $"Instance spawn group ({instanceMapId},{spawnGroupId}) FLAG_ALLIANCE_ONLY and FLAG_HORDE_ONLY may not be used together in a single entry - truncated to {info.Flags}.");
                }
        ...
        public void LoadPlayerInfo()
        {
            ...
            // Load playercreate
            {
                ...
                do
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, $"Wrong race {currentrace} in `playercreateinfo` table, ignoring.");
                        continue;
                    }

                    if (!CliDB.ChrClassesStorage.ContainsKey(currentclass))
                    {
                        Log.outWarn(LogFilter.Sql, $"Wrong class {currentclass} in `playercreateinfo` table, ignoring.");
                        continue;
                    }
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, $"Wrong home position for class {currentclass} race {currentrace} pair in `playercreateinfo` table, ignoring.");
                        continue;
                    }

                    if (CliDB.MapStorage.LookupByKey(mapId).Instanceable())
                    {
                        Log.outWarn(LogFilter.Sql, $"Home position in instanceable map for class {currentclass} race {currentrace} pair in `playercreateinfo` table, ignoring.");
                        continue;
                    }

                    if (Global.DB2Mgr.GetChrModel((Race)currentrace, Gender.Male) == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Missing male model for race {currentrace}, ignoring.");
                        continue;
                    }

                    if (Global.DB2Mgr.GetChrModel((Race)currentrace, Gender.Female) == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Missing female model for race {currentrace}, ignoring.");
                        continue;
                    }
                    ...
                    if (!result.IsNull(7))
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, $"Invalid NPE map id {info.createPositionNPE.Value.Loc.GetMapId()} for class {currentclass} race {currentrace} pair in `playercreateinfo` table, ignoring.");
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, $"Invalid NPE transport spawn id {info.createPositionNPE.Value.TransportGuid.Value} for class {currentclass} race {currentrace} pair in `playercreateinfo` table, ignoring.");
                    ...
                    if (!result.IsNull(13))
                    {
                        ...
                        if (CliDB.MovieStorage.ContainsKey(introMovieId))
                            ...
                            Log.outWarn(LogFilter.Sql, $"Invalid intro movie id {introMovieId} for class {currentclass} race {currentrace} pair in `playercreateinfo` table, ignoring.");
                    }
                    ...
                    if (!result.IsNull(14))
                    {
                        ...
                        if (GetSceneTemplate(introSceneId) != null)
                            ...
                            Log.outWarn(LogFilter.Sql, $"Invalid intro scene id {introSceneId} for class {currentclass} race {currentrace} pair in `playercreateinfo` table, ignoring.");
                    }
                    ...
                    if (!result.IsNull(15))
                    {
                        ...
                        if (GetSceneTemplate(introSceneId) != null)
                            ...
                            Log.outWarn(LogFilter.Sql, $"Invalid NPE intro scene id {introSceneId} for class {currentclass} race {currentrace} pair in `playercreateinfo` table, ignoring.");
                    }
            ...
            Log.outInfo(LogFilter.ServerLoading, "Loading Player Create Items Override Data...");
            {
                ...
                else
                {
                    ...
                    do
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Wrong race {0} in `playercreateinfo_item` table, ignoring.", currentrace);
                            continue;
                        }
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Wrong class {0} in `playercreateinfo_item` table, ignoring.", currentclass);
                            continue;
                        }
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Item id {0} (race {1} class {2}) in `playercreateinfo_item` table but not listed in `itemtemplate`, ignoring.", itemid, currentrace, currentclass);
                            continue;
                        }
                        ...
                        if (amount == 0)
                        {
                            Log.outWarn(LogFilter.Sql, "Item id {0} (class {1} race {2}) have amount == 0 in `playercreateinfo_item` table, ignoring.", itemid, currentrace, currentclass);
                            continue;
                        }
            ...
            Log.outInfo(LogFilter.ServerLoading, "Loading Player Create Custom Spell Data...");
            {
                ...
                else
                {
                    ...
                    do
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Wrong race mask {0} in `playercreateinfo_spell_custom` table, ignoring.", raceMask);
                            continue;
                        }

                        if (classMask != 0 && !Convert.ToBoolean(classMask & (int)Class.ClassMaskAllPlayable))
                        {
                            Log.outWarn(LogFilter.Sql, "Wrong class mask {0} in `playercreateinfo_spell_custom` table, ignoring.", classMask);
                            continue;
                        }
            ...
            Log.outInfo(LogFilter.ServerLoading, "Loading Player Create Cast Spell Data...");
            {
                ...
                else
                {
                    ...
                    do
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, $"Wrong race mask {raceMask} in `playercreateinfo_cast_spell` table, ignoring.");
                            continue;
                        }

                        if (classMask != 0 && !classMask.HasAnyFlag((uint)Class.ClassMaskAllPlayable))
                        {
                            Log.outWarn(LogFilter.Sql, $"Wrong class mask {classMask} in `playercreateinfo_cast_spell` table, ignoring.");
                            continue;
                        }

                        if (playerCreateMode < 0 || playerCreateMode >= (sbyte)PlayerCreateMode.Max)
                        {
                            Log.outWarn(LogFilter.Sql, $"Uses invalid createMode {playerCreateMode} in `playercreateinfo_cast_spell` table, ignoring.");
                            continue;
                        }
            ...
            Log.outInfo(LogFilter.ServerLoading, "Loading Player Create Action Data...");
            {
                ...
                else
                {
                    ...
                    do
                    {
                        ...
                        if (currentrace >= (int)Race.Max)
                        {
                            Log.outWarn(LogFilter.Sql, "Wrong race {0} in `playercreateinfo_action` table, ignoring.", currentrace);
                            continue;
                        }
                        ...
                        if (currentclass >= (int)Class.Max)
                        {
                            Log.outWarn(LogFilter.Sql, "Wrong class {0} in `playercreateinfo_action` table, ignoring.", currentclass);
                            continue;
                        }
            ...
            Log.outInfo(LogFilter.ServerLoading, "Loading Player Create Level Stats Data...");
            {
                ...
                for (var i = 0; i < (int)Race.Max; ++i)
                    raceStatModifiers[i] = new short[(int)CharacterStats.Max+2];    // 2 extras for base mana and hp


                //                                        0     1    2    3    4     5     6       7 
                SQLResult result = DB.World.Query("SELECT race, str, agi, sta, inte, spir FROM player_racestats");
                if (result.IsEmpty())
                {
                ...
                do
                {
                    ...
                    if (currentrace >= (int)Race.Max)
                    {
                        Log.outWarn(LogFilter.Sql, $"Wrong race {currentrace} in `player_racestats` table, ignoring.");
                        continue;
                    }

                    for (int i = 0; i < (int)CharacterStats.Max; ++i) {
                ...
                //                               0      1      2    3    4    5     6
                result = DB.World.Query("SELECT class, level, str, agi, sta, inte, spir, basehp, basemana FROM player_classlevelstats");
                if (result.IsEmpty())
                {
                ...
                do
                {
                    ...
                    if (currentclass >= (int)Class.Max)
                    {
                        Log.outWarn(LogFilter.Sql, "Wrong class {0} in `player_classlevelstats` table, ignoring.", currentclass);
                        continue;
                    }
                    ...
                    if (currentlevel > WorldConfig.GetIntValue(WorldCfg.MaxPlayerLevel))
                    {
                        if (currentlevel > 255)        // hardcoded level maximum
                            Log.outWarn(LogFilter.Sql, $"Wrong (> 255) level {currentlevel} in `player_classlevelstats` table, ignoring.");
                        else
                    ...
                    for (var race = 0; race < raceStatModifiers.Length; ++race)
                    {
                        ...
                        for (var i = 0; i < (int)CharacterStats.Max+2; i++)
                ...
                for (uint race = 0; race < (int)Race.Max; ++race)
                {
                    ...
                    for (uint _class = 0; _class < (int)Class.Max; ++_class)
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Race {0} Class {1} Level 1 does not have stats data!", race, _class);
                            Global.WorldMgr.StopNow();
                            return;
                        }
                        ...
                        for (var level = 1; level < WorldConfig.GetIntValue(WorldCfg.MaxPlayerLevel); ++level)
                        {
                            ...
                            {
                                Log.outWarn(LogFilter.Sql, "Race {0} Class {1} Level {2} does not have stats data. Using stats data of level {3}.", race, _class, level + 1, level);
            ...
            Log.outInfo(LogFilter.ServerLoading, "Loading Player Create XP Data...");
            {
                ...
                if (!result.IsEmpty())
                {
                    do
                    {
                        ...
                        if (currentlevel >= WorldConfig.GetIntValue(WorldCfg.MaxPlayerLevel))
                        {
                            if (currentlevel > SharedConst.StrongMaxLevel)        // hardcoded level maximum
                                Log.outWarn(LogFilter.Sql, "Wrong (> {0}) level {1} in `player_xp_for_level` table, ignoring.", 255, currentlevel);
                            else
                            {
                    ...
                    for (var level = 1; level < WorldConfig.GetIntValue(WorldCfg.MaxPlayerLevel); ++level)
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Level {0} does not have XP for level data. Using data of level [{1}] + 12000.", level + 1, level);
        ...
        void PlayerCreateInfoAddItemHelper(uint race, uint class_, uint itemId, int count)
        {
            ...
            {
                if (count < -1)
                    Log.outWarn(LogFilter.Sql, "Invalid count {0} specified on item {1} be removed from original player create info (use -1)!", count, itemId);
        ...
        PlayerLevelInfo BuildPlayerLevelInfo(Race race, Class _class, uint level)
        {
            ...
            for (int lvl = WorldConfig.GetIntValue(WorldCfg.MaxPlayerLevel) - 1; lvl < level; ++lvl)
            {
                ...
                {
                    case Class.Warrior:
                        info.stats[(int)CharacterStats.Strength] += (lvl > 23 ? 2 : (lvl > 1 ? 1 : 0));
                        info.stats[(int)CharacterStats.Stamina] += (lvl > 23 ? 2 : (lvl > 1 ? 1 : 0));
                        info.stats[(int)CharacterStats.Agility] += (lvl > 36 ? 1 : (lvl > 6 && (lvl % 2) != 0 ? 1 : 0));
                        info.stats[(int)CharacterStats.Intellect] += (lvl > 9 && (lvl % 2) == 0 ? 1 : 0);
                        info.stats[(int)CharacterStats.Spirit] += (lvl > 9 && (lvl % 2) == 0 ? 1 : 0);
                        break;
                    case Class.Paladin:
                        info.stats[(int)CharacterStats.Strength] += (lvl > 3 ? 1 : 0);
                        info.stats[(int)CharacterStats.Stamina] += (lvl > 33 ? 2 : (lvl > 1 ? 1 : 0));
                        info.stats[(int)CharacterStats.Agility] += (lvl > 38 ? 1 : (lvl > 7 && (lvl % 2) == 0 ? 1 : 0));
                        info.stats[(int)CharacterStats.Intellect] += (lvl > 6 && (lvl % 2) != 0 ? 1 : 0);
                        info.stats[(int)CharacterStats.Spirit] += (lvl > 7 ? 1 : 0);
                        break;
                    case Class.Hunter:
                        info.stats[(int)CharacterStats.Strength] += (lvl > 4 ? 1 : 0);
                        info.stats[(int)CharacterStats.Stamina] += (lvl > 4 ? 1 : 0);
                        info.stats[(int)CharacterStats.Agility] += (lvl > 33 ? 2 : (lvl > 1 ? 1 : 0));
                        info.stats[(int)CharacterStats.Intellect] += (lvl > 8 && (lvl % 2) != 0 ? 1 : 0);
                        info.stats[(int)CharacterStats.Spirit] += (lvl > 38 ? 1 : (lvl > 9 && (lvl % 2) != 0 ? 1 : 0));
                        break;
                    case Class.Rogue:
                        info.stats[(int)CharacterStats.Strength] += (lvl > 5 ? 1 : 0);
                        info.stats[(int)CharacterStats.Stamina] += (lvl > 4 ? 1 : 0);
                        info.stats[(int)CharacterStats.Agility] += (lvl > 16 ? 2 : (lvl > 1 ? 1 : 0));
                        info.stats[(int)CharacterStats.Intellect] += (lvl > 16 ? 2 : (lvl > 1 ? 1 : 0));
                        info.stats[(int)CharacterStats.Spirit] += (lvl > 8 && (lvl % 2) == 0 ? 1 : 0);
                        break;
                    case Class.Priest:
                        info.stats[(int)CharacterStats.Strength] += (lvl > 9 && (lvl % 2) == 0 ? 1 : 0);
                        info.stats[(int)CharacterStats.Stamina] += (lvl > 5 ? 1 : 0);
                        info.stats[(int)CharacterStats.Agility] += (lvl > 38 ? 1 : (lvl > 8 && (lvl % 2) != 0 ? 1 : 0));
                        info.stats[(int)CharacterStats.Intellect] += (lvl > 22 ? 2 : (lvl > 1 ? 1 : 0));
                        info.stats[(int)CharacterStats.Spirit] += (lvl > 3 ? 1 : 0);
                        break;
                    case Class.Shaman:
                        info.stats[(int)CharacterStats.Strength] += (lvl > 34 ? 1 : (lvl > 6 && (lvl % 2) != 0 ? 1 : 0));
                        info.stats[(int)CharacterStats.Stamina] += (lvl > 4 ? 1 : 0);
                        info.stats[(int)CharacterStats.Agility] += (lvl > 7 && (lvl % 2) == 0 ? 1 : 0);
                        info.stats[(int)CharacterStats.Intellect] += (lvl > 5 ? 1 : 0);
                        info.stats[(int)CharacterStats.Spirit] += (lvl > 4 ? 1 : 0);
                        break;
                    case Class.Mage:
                        info.stats[(int)CharacterStats.Strength] += (lvl > 9 && (lvl % 2) == 0 ? 1 : 0);
                        info.stats[(int)CharacterStats.Stamina] += (lvl > 5 ? 1 : 0);
                        info.stats[(int)CharacterStats.Agility] += (lvl > 9 && (lvl % 2) == 0 ? 1 : 0);
                        info.stats[(int)CharacterStats.Intellect] += (lvl > 24 ? 2 : (lvl > 1 ? 1 : 0));
                        info.stats[(int)CharacterStats.Spirit] += (lvl > 33 ? 2 : (lvl > 2 ? 1 : 0));
                        break;
                    case Class.Warlock:
                        info.stats[(int)CharacterStats.Strength] += (lvl > 9 && (lvl % 2) == 0 ? 1 : 0);
                        info.stats[(int)CharacterStats.Stamina] += (lvl > 38 ? 2 : (lvl > 3 ? 1 : 0));
                        info.stats[(int)CharacterStats.Agility] += (lvl > 9 && (lvl % 2) == 0 ? 1 : 0);
                        info.stats[(int)CharacterStats.Intellect] += (lvl > 33 ? 2 : (lvl > 2 ? 1 : 0));
                        info.stats[(int)CharacterStats.Spirit] += (lvl > 38 ? 2 : (lvl > 3 ? 1 : 0));
                        break;
                    case Class.Druid:
                        info.stats[(int)CharacterStats.Strength] += (lvl > 38 ? 2 : (lvl > 6 && (lvl % 2) != 0 ? 1 : 0));
                        info.stats[(int)CharacterStats.Stamina] += (lvl > 32 ? 2 : (lvl > 4 ? 1 : 0));
                        info.stats[(int)CharacterStats.Agility] += (lvl > 38 ? 2 : (lvl > 8 && (lvl % 2) != 0 ? 1 : 0));
                        info.stats[(int)CharacterStats.Intellect] += (lvl > 38 ? 3 : (lvl > 4 ? 1 : 0));
                        info.stats[(int)CharacterStats.Spirit] += (lvl > 38 ? 3 : (lvl > 5 ? 1 : 0));
                        break;
                }
            }
        ...
        public void LoadPetLevelInfo()
        {
            ...
            do
            {
                ...
                if (GetCreatureTemplate(creatureid) == null)
                {
                    Log.outWarn(LogFilter.Sql, "Wrong creature id {0} in `pet_levelstats` table, ignoring.", creatureid);
                    continue;
                }
                ...
                {
                    if (currentlevel > SharedConst.StrongMaxLevel)        // hardcoded level maximum
                        Log.outWarn(LogFilter.Sql, "Wrong (> {0}) level {1} in `pet_levelstats` table, ignoring.", SharedConst.StrongMaxLevel, currentlevel);
                    else
                    {
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Wrong (<1) level {0} in `pet_levelstats` table, ignoring.", currentlevel);
                    continue;
                }
            ...
            foreach (var map in petInfoStore)
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Creature {0} does not have pet stats data for Level 1!", map.Key);
                    Global.WorldMgr.StopNow();
                }
                ...
                for (byte level = 1; level < WorldConfig.GetIntValue(WorldCfg.MaxPlayerLevel); ++level)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Creature {0} has no data for Level {1} pet stats data, using data of Level {2}.", map.Key, level + 1, level);
        ...
        public void LoadFactionChangeAchievements()
        {
            ...
            do
            {
                ...
                if (!CliDB.AchievementStorage.ContainsKey(alliance))
                    Log.outWarn(LogFilter.Sql, "Achievement {0} (alliance_id) referenced in `player_factionchange_achievement` does not exist, pair skipped!", alliance);
                else if (!CliDB.AchievementStorage.ContainsKey(horde))
                    Log.outWarn(LogFilter.Sql, "Achievement {0} (horde_id) referenced in `player_factionchange_achievement` does not exist, pair skipped!", horde);
                else
        ...
        public void LoadFactionChangeQuests()
        {
            ...
            do
            {
                ...
                if (Global.ObjectMgr.GetQuestTemplate(alliance) == null)
                    Log.outWarn(LogFilter.Sql, "Quest {0} (alliance_id) referenced in `player_factionchange_quests` does not exist, pair skipped!", alliance);
                else if (Global.ObjectMgr.GetQuestTemplate(horde) == null)
                    Log.outWarn(LogFilter.Sql, "Quest {0} (horde_id) referenced in `player_factionchange_quests` does not exist, pair skipped!", horde);
                else
        ...
        public void LoadFactionChangeReputations()
        {
            ...
            do
            {
                ...
                if (!CliDB.FactionStorage.ContainsKey(alliance))
                    Log.outWarn(LogFilter.Sql, "Reputation {0} (alliance_id) referenced in `player_factionchange_reputations` does not exist, pair skipped!", alliance);
                else if (!CliDB.FactionStorage.ContainsKey(horde))
                    Log.outWarn(LogFilter.Sql, "Reputation {0} (horde_id) referenced in `player_factionchange_reputations` does not exist, pair skipped!", horde);
                else
        ...
        public void LoadFactionChangeSpells()
        {
            ...
            do
            {
                ...
                if (!Global.SpellMgr.HasSpellInfo(alliance, Difficulty.None))
                    Log.outWarn(LogFilter.Sql, "Spell {0} (alliance_id) referenced in `player_factionchange_spells` does not exist, pair skipped!", alliance);
                else if (!Global.SpellMgr.HasSpellInfo(horde, Difficulty.None))
                    Log.outWarn(LogFilter.Sql, "Spell {0} (horde_id) referenced in `player_factionchange_spells` does not exist, pair skipped!", horde);
                else
        ...
        public void LoadFactionChangeTitles()
        {
            ...
            do
            {
                ...
                if (!CliDB.CharTitlesStorage.ContainsKey(alliance))
                    Log.outWarn(LogFilter.Sql, "Title {0} (alliance_id) referenced in `player_factionchange_title` does not exist, pair skipped!", alliance);
                else if (!CliDB.CharTitlesStorage.ContainsKey(horde))
                    Log.outWarn(LogFilter.Sql, "Title {0} (horde_id) referenced in `player_factionchange_title` does not exist, pair skipped!", horde);
                else
        ...
        public void LoadQuests()
        {
            ...
            else
            {
                do
                {
                    ...
                    if (quest != null)
                        ...
                        Log.outWarn(LogFilter.Sql, $"Table `quest_reward_choice_items` has data for quest {questId} but such quest does not exist");
                } while (result.NextRow());
            }
            ...
            else
            {
                do
                {
                    ...
                    if (quest != null)
                        ...
                        Log.outWarn(LogFilter.Sql, $"Table `quest_reward_display_spell` has data for quest {questId} but such quest does not exist");
                } while (result.NextRow());
            }
            ...
            else
            {
                do
                {
                    ...
                    if (quest != null)
                        ...
                        Log.outWarn(LogFilter.Sql, "Table `quest_details` has data for quest {0} but such quest does not exist", questId);
                } while (result.NextRow());
            }
            ...
            else
            {
                do
                {
                    ...
                    if (quest != null)
                        ...
                        Log.outWarn(LogFilter.Sql, "Table `quest_request_items` has data for quest {0} but such quest does not exist", questId);
                } while (result.NextRow());
            }
            ...
            else
            {
                do
                {
                    ...
                    if (quest != null)
                        ...
                        Log.outWarn(LogFilter.Sql, "Table `quest_offer_reward` has data for quest {0} but such quest does not exist", questId);
                } while (result.NextRow());
            }
            ...
            else
            {
                do
                {
                    ...
                    if (quest != null)
                        ...
                        Log.outWarn(LogFilter.Sql, "Table `quest_template_addon` has data for quest {0} but such quest does not exist", questId);
                } while (result.NextRow());
            }
            ...
            else
            {
                do
                {
                    ...
                    if (quest != null)
                        ...
                        Log.outWarn(LogFilter.Sql, "Table `quest_mail_sender` has data for quest {0} but such quest does not exist", questId);
                } while (result.NextRow());
            }
            ...
            else
            {
                do
                {
                    ...
                    if (quest != null)
                        ...
                        Log.outWarn(LogFilter.Sql, "Table `quest_objectives` has objective for quest {0} but such quest does not exist", questId);
                } while (result.NextRow());
            }
            ...
            else
            {
                do
                {
                    ...
                    if (vID == 0)
                    {
                        Log.outWarn(LogFilter.Sql, "Table `quest_visual_effect` has visual effect for null objective id");
                        continue;
                    }
                    ...
                    if (vID != oID)
                    {
                        Log.outWarn(LogFilter.Sql, "Table `quest_visual_effect` has visual effect for objective {0} but such objective does not exist.", vID);
                        continue;
                    }
            ...
            foreach (var qinfo in _questTemplates.Values)
            {
                ...
                if (qinfo.Type >= QuestType.Max)
                    Log.outWarn(LogFilter.Sql, "Quest {0} has `Method` = {1}, expected values are 0, 1 or 2.", qinfo.Id, qinfo.Type);

                if (Convert.ToBoolean(qinfo.SpecialFlags & ~QuestSpecialFlags.DbAllowed))
                {
                    Log.outWarn(LogFilter.Sql, "Quest {0} has `SpecialFlags` = {1} > max allowed value. Correct `SpecialFlags` to value <= {2}",
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Weekly Quest {0} is marked as daily quest in `Flags`, removed daily flag.", qinfo.Id);
                ...
                if (qinfo.Flags.HasAnyFlag(QuestFlags.Daily))
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Daily Quest {0} not marked as repeatable in `SpecialFlags`, added.", qinfo.Id);
                ...
                if (qinfo.Flags.HasAnyFlag(QuestFlags.Weekly))
                {
                    if (!qinfo.SpecialFlags.HasAnyFlag(QuestSpecialFlags.Repeatable))
                    {
                        Log.outWarn(LogFilter.Sql, "Weekly Quest {0} not marked as repeatable in `SpecialFlags`, added.", qinfo.Id);
                        qinfo.SpecialFlags |= QuestSpecialFlags.Repeatable;
                    }
                }
                ...
                if (qinfo.SpecialFlags.HasAnyFlag(QuestSpecialFlags.Monthly))
                {
                    if (!qinfo.SpecialFlags.HasAnyFlag(QuestSpecialFlags.Repeatable))
                    {
                        Log.outWarn(LogFilter.Sql, "Monthly quest {0} not marked as repeatable in `SpecialFlags`, added.", qinfo.Id);
                        qinfo.SpecialFlags |= QuestSpecialFlags.Repeatable;
                    }
                }
                ...
                if (Convert.ToBoolean(qinfo.Flags & QuestFlags.Tracking))
                {
                    ...
                    for (int j = 1; j < qinfo.RewardChoiceItemId.Length; ++j)
                    {
                        ...
                        if (id != 0)
                        {
                            Log.outWarn(LogFilter.Sql, "Quest {0} has `RewardChoiceItemId{1}` = {2} but item from `RewardChoiceItemId{3}` can't be rewarded with quest flag QUESTFLAGSTRACKING.",
                                qinfo.Id, j + 1, id, j + 1);
                ...
                if (qinfo.ContentTuningId != 0 && !CliDB.ContentTuningStorage.ContainsKey(qinfo.ContentTuningId))
                    Log.outWarn(LogFilter.Sql, $"Quest {qinfo.Id} has `ContentTuningID` = {qinfo.ContentTuningId} but content tuning with this id does not exist.");
                ...
                if (qinfo.QuestSortID > 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Quest {0} has `ZoneOrSort` = {1} (zone case) but zone with this id does not exist.",
                ...
                if (qinfo.QuestSortID < 0)
                {
                    ...
                    if (qSort == null)
                    {
                        Log.outWarn(LogFilter.Sql, "Quest {0} has `ZoneOrSort` = {1} (sort case) but quest sort with this id does not exist.",
                            qinfo.Id, qinfo.QuestSortID);
                    ...
                    if (skillid != SkillType.None)
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Quest {0} has `ZoneOrSort` = {1} but `RequiredSkillId` does not have a corresponding value ({2}).",
                ...
                if (qinfo.AllowableClasses != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Quest {0} does not contain any playable classes in `RequiredClasses` ({1}), value set to 0 (all classes).", qinfo.Id, qinfo.AllowableClasses);
                ...
                if (qinfo.AllowableRaces != -1)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Quest {0} does not contain any playable races in `RequiredRaces` ({1}), value set to 0 (all races).", qinfo.Id, qinfo.AllowableRaces);
                ...
                if (qinfo.RequiredSkillId != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Quest {0} has `RequiredSkillId` = {1} but this skill does not exist",
                ...
                if (qinfo.RequiredSkillPoints != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Quest {0} has `RequiredSkillPoints` = {1} but max possible skill is {2}, quest can't be done.",
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Quest {0} has `RequiredMinRepFaction` = {1} but faction template {2} does not exist, quest can't be done.",
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Quest {0} has `RequiredMaxRepFaction` = {1} but faction template {2} does not exist, quest can't be done.",
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Quest {0} has `RequiredMinRepValue` = {1} but max reputation is {2}, quest can't be done.",
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Quest {0} has `RequiredMaxRepValue` = {1} and `RequiredMinRepValue` = {2}, quest can't be done.",
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Quest {0} has `RequiredMinRepValue` = {1} but `RequiredMinRepFaction` is 0, value has no effect",
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Quest {0} has `RequiredMaxRepValue` = {1} but `RequiredMaxRepFaction` is 0, value has no effect",
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Quest {0} has `RewardTitleId` = {1} but CharTitle Id {1} does not exist, quest can't be rewarded with title.",
                ...
                if (qinfo.SourceItemId != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Quest {0} has `SourceItemId` = {1} but item with entry {2} does not exist, quest can't be done.",
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Quest {0} has `StartItem` = {1} but `ProvidedItemCount` = 0, set to 1 but need fix in DB.",
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Quest {0} has `SourceItemId` = 0 but `SourceItemIdCount` = {1}, useless value.",
                ...
                if (qinfo.SourceSpellID != 0)
                {
                    ...
                    if (spellInfo == null)
                    {
                        Log.outWarn(LogFilter.Sql, "Quest {0} has `SourceSpellid` = {1} but spell {1} doesn't exist, quest can't be done.",
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Quest {0} has `SourceSpellid` = {1} but spell {1} is broken, quest can't be done.",
                            qinfo.Id, qinfo.SourceSpellID);
                        qinfo.SourceSpellID = 0;                        // quest can't be done for this requirement
                    }
                }
                    ...
                    if (obj.StorageIndex < 0)
                    {
                        switch (obj.Type)
                        {
                            ...
                            case QuestObjectiveType.ObtainCurrency:
                                Log.outWarn(LogFilter.Sql, "Quest {0} objective {1} has invalid StorageIndex = {2} for objective type {3}", qinfo.Id, obj.Id, obj.StorageIndex, obj.Type);
                                break;
                            default:
                                break;
                        }
                    ...
                    switch (obj.Type)
                    {
                        case QuestObjectiveType.Item:
                            if (GetItemTemplate((uint)obj.ObjectID) == null)
                                Log.outWarn(LogFilter.Sql, $"Quest {qinfo.Id} objective {obj.Id} has non existing item entry {obj.ObjectID}, quest can't be done.");
                            break;
                        case QuestObjectiveType.Monster:
                            if (GetCreatureTemplate((uint)obj.ObjectID) == null)
                                Log.outWarn(LogFilter.Sql, $"Quest {qinfo.Id} objective {obj.Id} has non existing creature entry {obj.ObjectID}, quest can't be done.");
                            break;
                        case QuestObjectiveType.GameObject:
                            if (GetGameObjectTemplate((uint)obj.ObjectID) == null)
                                Log.outWarn(LogFilter.Sql, $"Quest {qinfo.Id} objective {obj.Id} has non existing gameobject entry {obj.ObjectID}, quest can't be done.");
                            break;
                        case QuestObjectiveType.TalkTo:
                        ...
                        case QuestObjectiveType.IncreaseReputation:
                            if (!CliDB.FactionStorage.ContainsKey((uint)obj.ObjectID))
                                Log.outWarn(LogFilter.Sql, "Quest {0} objective {1} has non existing faction id {2}", qinfo.Id, obj.Id, obj.ObjectID);
                            break;
                        case QuestObjectiveType.PlayerKills:
                            if (obj.Amount <= 0)
                                Log.outWarn(LogFilter.Sql, "Quest {0} objective {1} has invalid player kills count {2}", qinfo.Id, obj.Id, obj.Amount);
                            break;
                        case QuestObjectiveType.Currency:
                        case QuestObjectiveType.HaveCurrency:
                        case QuestObjectiveType.ObtainCurrency:
                            if (!CliDB.CurrencyTypesStorage.ContainsKey((uint)obj.ObjectID))
                                Log.outWarn(LogFilter.Sql, "Quest {0} objective {1} has non existing currency {2}", qinfo.Id, obj.Id, obj.ObjectID);
                            if (obj.Amount <= 0)
                                Log.outWarn(LogFilter.Sql, "Quest {0} objective {1} has invalid currency amount {2}", qinfo.Id, obj.Id, obj.Amount);
                            break;
                        case QuestObjectiveType.LearnSpell:
                            if (!Global.SpellMgr.HasSpellInfo((uint)obj.ObjectID, Difficulty.None))
                                Log.outWarn(LogFilter.Sql, "Quest {0} objective {1} has non existing spell id {2}", qinfo.Id, obj.Id, obj.ObjectID);
                            break;
                        case QuestObjectiveType.WinPetBattleAgainstNpc:
                            if (obj.ObjectID != 0 && Global.ObjectMgr.GetCreatureTemplate((uint)obj.ObjectID) == null)
                                Log.outWarn(LogFilter.Sql, "Quest {0} objective {1} has non existing creature entry {2}, quest can't be done.", qinfo.Id, obj.Id, obj.ObjectID);
                            break;
                        case QuestObjectiveType.DefeatBattlePet:
                            if (!CliDB.BattlePetSpeciesStorage.ContainsKey((uint)obj.ObjectID))
                                Log.outWarn(LogFilter.Sql, "Quest {0} objective {1} has non existing battlepet species id {2}", qinfo.Id, obj.Id, obj.ObjectID);
                            break;
                        case QuestObjectiveType.CriteriaTree:
                            if (!CliDB.CriteriaTreeStorage.ContainsKey((uint)obj.ObjectID))
                                Log.outWarn(LogFilter.Sql, "Quest {0} objective {1} has non existing criteria tree id {2}", qinfo.Id, obj.Id, obj.ObjectID);
                            break;
                        case QuestObjectiveType.AreaTrigger:
                            if (!CliDB.AreaTriggerStorage.ContainsKey((uint)obj.ObjectID) && obj.ObjectID != -1)
                                Log.outWarn(LogFilter.Sql, "Quest {0} objective {1} has non existing AreaTrigger.db2 id {2}", qinfo.Id, obj.Id, obj.ObjectID);
                            break;
                        ...
                        case QuestObjectiveType.AreaTriggerExit:
                            if (Global.AreaTriggerDataStorage.GetAreaTriggerTemplate(new AreaTriggerId((uint)obj.ObjectID, false)) == null && Global.AreaTriggerDataStorage.GetAreaTriggerTemplate(new AreaTriggerId((uint)obj.ObjectID, true)) != null)
                                Log.outWarn(LogFilter.Sql, "Quest {0} objective {1} has non existing areatrigger id {2}", qinfo.Id, obj.Id, obj.ObjectID);
                            break;
                        ...
                        case QuestObjectiveType.ProgressBar:
                            break;
                        default:
                            Log.outWarn(LogFilter.Sql, "Quest {0} objective {1} has unhandled type {2}", qinfo.Id, obj.Id, obj.Type);
                            break;
                    }
                ...
                for (var j = 0; j < SharedConst.QuestItemDropCount; j++)
                {
                    ...
                    if (id != 0)
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Quest {0} has `RequiredSourceItemId{1}` = {2} but item with entry {2} does not exist, quest can't be done.",
                                qinfo.Id, j + 1, id);
                            // no changes, quest can't be done for this requirement
                        }
                    }
                    else
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Quest {0} has `RequiredSourceItemId{1}` = 0 but `RequiredSourceItemCount{1}` = {2}.",
                    ...
                    if (id != 0)
                    {
                        switch (qinfo.RewardChoiceItemType[j])
                        {
                            case LootItemType.Item:
                                ...
                                {
                                    Log.outWarn(LogFilter.Sql, $"Quest {qinfo.Id} has `RewardChoiceItemId{j + 1}` = {id} but item with entry {id} does not exist, quest will not reward this item.");
                            ...
                            case LootItemType.Currency:
                                ...
                                {
                                    Log.outWarn(LogFilter.Sql, $"Quest {qinfo.Id} has `RewardChoiceItemId{j + 1}` = {id} but currency with id {id} does not exist, quest will not reward this currency.");
                                    qinfo.RewardChoiceItemId[j] = 0;          // no changes, quest will not reward this
                                }
                                break;
                            default:
                                Log.outWarn(LogFilter.Sql, $"Quest {qinfo.Id} has `RewardChoiceItemType{j + 1}` = {qinfo.RewardChoiceItemType[j]} but it is not a valid item type, reward removed.");
                                qinfo.RewardChoiceItemId[j] = 0;
                                break;
                        }

                        if (qinfo.RewardChoiceItemCount[j] == 0)
                            Log.outWarn(LogFilter.Sql, $"Quest {qinfo.Id} has `RewardChoiceItemId{j + 1}` = {id} but `RewardChoiceItemCount{j + 1}` = 0, quest can't be done.");
                    }
                    else if (qinfo.RewardChoiceItemCount[j] > 0)
                    {
                        Log.outWarn(LogFilter.Sql, $"Quest {qinfo.Id} has `RewardChoiceItemId{j + 1}` = 0 but `RewardChoiceItemCount{j + 1}` = {qinfo.RewardChoiceItemCount[j]}.");
                        // no changes, quest ignore this data
                    }
                }
                ...
                for (var j = 0; j < SharedConst.QuestRewardItemCount; ++j)
                {
                    ...
                    if (id != 0)
                    {
                        if (Global.ObjectMgr.GetItemTemplate(id) == null)
                        {
                            Log.outWarn(LogFilter.Sql, "Quest {0} has `RewardItemId{1}` = {2} but item with entry {3} does not exist, quest will not reward this item.",
                                qinfo.Id, j + 1, id, id);
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Quest {0} has `RewardItemId{1}` = {2} but `RewardItemIdCount{3}` = 0, quest will not reward this item.",
                                qinfo.Id, j + 1, id, j + 1);
                            // no changes
                        }
                    }
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Quest {0} has `RewardItemId{1}` = 0 but `RewardItemIdCount{2}` = {3}.",
                ...
                for (var j = 0; j < SharedConst.QuestRewardReputationsCount; ++j)
                {
                    if (qinfo.RewardFactionId[j] != 0)
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Quest {0} has RewardFactionValueId{1} = {2}. That is outside the range of valid values (-9 to 9).", qinfo.Id, j + 1, qinfo.RewardFactionValue[j]);
                        }
                        if (!CliDB.FactionStorage.ContainsKey(qinfo.RewardFactionId[j]))
                        {
                            Log.outWarn(LogFilter.Sql, "Quest {0} has `RewardFactionId{1}` = {2} but raw faction (faction.dbc) {3} does not exist, quest will not reward reputation for this faction.",
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Quest {0} has `RewardFactionId{1}` = 0 but `RewardFactionValueIdOverride{2}` = {3}.",
                ...
                if (qinfo.RewardSpell > 0)
                {
                    ...
                    if (spellInfo == null)
                    {
                        Log.outWarn(LogFilter.Sql, "Quest {0} has `RewardSpellCast` = {1} but spell {2} does not exist, quest will not have a spell reward.",
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Quest {0} has `RewardSpellCast` = {1} but spell {2} is broken, quest will not have a spell reward.",
                            qinfo.Id, qinfo.RewardSpell, qinfo.RewardSpell);
                        qinfo.RewardSpell = 0;                    // no spell will be casted on player
                    }
                }
                ...
                if (qinfo.RewardMailTemplateId != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Quest {0} has `RewardMailTemplateId` = {1} but mail template {2} does not exist, quest will not have a mail reward.",
                    ...
                    else if (usedMailTemplates.ContainsKey(qinfo.RewardMailTemplateId))
                    {
                        ...
                        Log.outWarn(LogFilter.Sql, "Quest {0} has `RewardMailTemplateId` = {1} but mail template  {2} already used for quest {3}, quest will not have a mail reward.",
                ...
                if (qinfo.NextQuestInChain != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Quest {0} has `NextQuestIdChain` = {1} but quest {2} does not exist, quest chain will not work.",
                ...
                for (var j = 0; j < SharedConst.QuestRewardCurrencyCount; ++j)
                {
                    if (qinfo.RewardCurrencyId[j] != 0)
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Quest {0} has `RewardCurrencyId{1}` = {2} but `RewardCurrencyCount{3}` = 0, quest can't be done.",
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Quest {0} has `RewardCurrencyId{1}` = {2} but currency with entry {3} does not exist, quest can't be done.",
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Quest {0} has `RewardCurrencyId{1}` = 0 but `RewardCurrencyCount{2}` = {3}, quest can't be done.",
                ...
                if (qinfo.SoundAccept != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Quest {0} has `SoundAccept` = {1} but sound {2} does not exist, set to 0.",
                ...
                if (qinfo.SoundTurnIn != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Quest {0} has `SoundTurnIn` = {1} but sound {2} does not exist, set to 0.",
                ...
                if (qinfo.RewardSkillId > 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Quest {0} has `RewardSkillId` = {1} but this skill does not exist",
                            qinfo.Id, qinfo.RewardSkillId);
                    }
                    if (qinfo.RewardSkillPoints == 0)
                    {
                        Log.outWarn(LogFilter.Sql, "Quest {0} has `RewardSkillId` = {1} but `RewardSkillPoints` is 0",
                            qinfo.Id, qinfo.RewardSkillId);
                    }
                }
                ...
                if (qinfo.RewardSkillPoints != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Quest {0} has `RewardSkillPoints` = {1} but max possible skill is {2}, quest can't be done.",
                            qinfo.Id, qinfo.RewardSkillPoints, Global.WorldMgr.GetConfigMaxSkillValue());
                        // no changes, quest can't be done for this requirement
                    }
                    if (qinfo.RewardSkillId == 0)
                    {
                        Log.outWarn(LogFilter.Sql, "Quest {0} has `RewardSkillPoints` = {1} but `RewardSkillId` is 0",
                ...
                if (prevQuestId != 0)
                {
                    ...
                    if (prevQuestItr == null)
                        Log.outWarn(LogFilter.Sql, $"Quest {qinfo.Id} has PrevQuestId {prevQuestId}, but no such quest");
                    else if (prevQuestItr.BreadcrumbForQuestId != 0)
                        Log.outWarn(LogFilter.Sql, $"Quest {qinfo.Id} should not be unlocked by breadcrumb quest {prevQuestId}");
                ...
                if (qinfo.NextQuestId != 0)
                {
                    ...
                    if (nextquest == null)
                        Log.outWarn(LogFilter.Sql, "Quest {0} has NextQuestId {1}, but no such quest", qinfo.Id, qinfo.NextQuestId);
                    else
                ...
                if (breadcrumbForQuestId != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, $"Quest {qinfo.Id} is a breadcrumb for quest {breadcrumbForQuestId}, but no such quest exists");
                        qinfo.BreadcrumbForQuestId = 0;
                    }
                    if (qinfo.NextQuestId != 0)
                        Log.outWarn(LogFilter.Sql, $"Quest {qinfo.Id} is a breadcrumb, should not unlock quest {qinfo.NextQuestId}");
                }
            ...
            foreach (var questPair in _questTemplates)
            {
                ...
                while (breadcrumbForQuestId != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, $"Breadcrumb quests {qid} and {breadcrumbForQuestId} are in a loop");
                        qinfo.BreadcrumbForQuestId = 0;
                        break;
                    }
            ...
            foreach (SpellNameRecord spellNameEntry in CliDB.SpellNameStorage.Values)
            {
                ...
                foreach (var spellEffectInfo in spellInfo.GetEffects())
                {
                    ...
                    if (!quest.HasSpecialFlag(QuestSpecialFlags.ExplorationOrEvent))
                    {
                        Log.outWarn(LogFilter.Sql, "Spell (id: {0}) have SPELL_EFFECT_QUEST_COMPLETE for quest {1}, but quest not have flag QUEST_SPECIAL_FLAGS_EXPLORATION_OR_EVENT. " +
        ...
        public void LoadGameobjectQuestStarters()
        {
            ...
            foreach (var pair in _goQuestRelations)
            {
                ...
                if (goInfo == null)
                    Log.outWarn(LogFilter.Sql, "Table `gameobject_queststarter` have data for not existed gameobject entry ({0}) and existed quest {1}", pair.Key, pair.Value);
                else if (goInfo.type != GameObjectTypes.QuestGiver)
                    Log.outWarn(LogFilter.Sql, "Table `gameobject_queststarter` have data gameobject entry ({0}) for quest {1}, but GO is not GAMEOBJECT_TYPE_QUESTGIVER", pair.Key, pair.Value);
            }
        }
        public void LoadGameobjectQuestEnders()
        {
            ...
            foreach (var pair in _goQuestInvolvedRelations)
            {
                GameObjectTemplate goInfo = GetGameObjectTemplate(pair.Key);
                if (goInfo == null)
                    Log.outWarn(LogFilter.Sql, "Table `gameobject_questender` have data for not existed gameobject entry ({0}) and existed quest {1}", pair.Key, pair.Value);
                else if (goInfo.type != GameObjectTypes.QuestGiver)
                    Log.outWarn(LogFilter.Sql, "Table `gameobject_questender` have data gameobject entry ({0}) for quest {1}, but GO is not GAMEOBJECT_TYPE_QUESTGIVER", pair.Key, pair.Value);
            }
        }
        public void LoadCreatureQuestStarters()
        {
            ...
            foreach (var pair in _creatureQuestRelations)
            {
                ...
                if (cInfo == null)
                    Log.outWarn(LogFilter.Sql, "Table `creature_queststarter` have data for not existed creature entry ({0}) and existed quest {1}", pair.Key, pair.Value);
                else if (!Convert.ToBoolean(cInfo.Npcflag & (uint)NPCFlags.QuestGiver))
                    Log.outWarn(LogFilter.Sql, "Table `creature_queststarter` has creature entry ({0}) for quest {1}, but npcflag does not include UNIT_NPC_FLAG_QUESTGIVER", pair.Key, pair.Value);
            }
        }
        public void LoadCreatureQuestEnders()
        {
            ...
            foreach (var pair in _creatureQuestInvolvedRelations)
            {
                CreatureTemplate cInfo = GetCreatureTemplate(pair.Key);
                if (cInfo == null)
                    Log.outWarn(LogFilter.Sql, "Table `creature_questender` have data for not existed creature entry ({0}) and existed quest {1}", pair.Key, pair.Value);
                else if (!Convert.ToBoolean(cInfo.Npcflag & (uint)NPCFlags.QuestGiver))
                    Log.outWarn(LogFilter.Sql, "Table `creature_questender` has creature entry ({0}) for quest {1}, but npcflag does not include UNIT_NPC_FLAG_QUESTGIVER", pair.Key, pair.Value);
            }
        }
        void LoadQuestRelationsHelper(MultiMap<uint, uint> map, MultiMap<uint, uint> reverseMap, string table)
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Table `{0}`: Quest {1} listed for entry {2} does not exist.", table, quest, id);
                    continue;
                }
        ...
        public void LoadQuestPOI()
        {
            ...
            do
            {
                ...
                if (Global.ObjectMgr.GetQuestTemplate(questID) == null)
                    Log.outWarn(LogFilter.Sql, $"`quest_poi` quest id ({questID}) Idx1 ({idx1}) does not exist in `quest_template`");
                ...
                Log.outWarn(LogFilter.Sql, $"Table quest_poi references unknown quest points for quest {questID} POI id {blobIndex}");
        ...
        public void LoadQuestAreaTriggers()
        {
            ...
            do
            {
                ...
                if (quest == null)
                {
                    Log.outWarn(LogFilter.Sql, "Table `areatrigger_involvedrelation` has record (id: {0}) for not existing quest {1}", trigger_ID, quest_ID);
                    continue;
                }

                if (!quest.HasSpecialFlag(QuestSpecialFlags.ExplorationOrEvent))
                {
                    Log.outWarn(LogFilter.Sql, "Table `areatrigger_involvedrelation` has record (id: {0}) for not quest {1}, but quest not have flag QUEST_SPECIAL_FLAGS_EXPLORATION_OR_EVENT. Trigger or quest flags must be fixed, quest modified to require objective.", trigger_ID, quest_ID);
            ...
            do
            {
                ...
                switch (type)
                {
                    case 0: // Creature
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `quest_greeting`: creature template entry {0} does not exist.", id);
                            continue;
                        }
                        break;
                    case 1: // GameObject
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `quest_greeting`: gameobject template entry {0} does not exist.", id);
                            continue;
                        }
                        break;
                    default:
        ...
        void LoadTerrainWorldMaps()
        {
            ...
            do
            {
                ...
                if (!CliDB.MapStorage.ContainsKey(mapId))
                {
                    Log.outWarn(LogFilter.Sql, "TerrainSwapMap {0} defined in `terrain_worldmap` does not exist, skipped.", mapId);
                    continue;
                }

                if (!Global.DB2Mgr.IsUiMapPhase((int)uiMapPhaseId))
                {
                    Log.outWarn(LogFilter.Sql, $"Phase {uiMapPhaseId} defined in `terrain_worldmap` is not a valid terrain swap phase, skipped.");
                    continue;
                }
        ...
        void LoadTerrainSwapDefaults()
        {
            ...
            do
            {
                ...
                if (!CliDB.MapStorage.ContainsKey(mapId))
                {
                    Log.outWarn(LogFilter.Sql, "Map {0} defined in `terrain_swap_defaults` does not exist, skipped.", mapId);
                    continue;
                }
                ...
                {
                    Log.outWarn(LogFilter.Sql, "TerrainSwapMap {0} defined in `terrain_swap_defaults` does not exist, skipped.", terrainSwap);
                    continue;
                }
        ...
        void LoadAreaPhases()
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"Area {area} defined in `phase_area` does not exist, skipped.");
                    continue;
                }

                if (!CliDB.PhaseStorage.ContainsKey(phaseId))
                {
                    Log.outWarn(LogFilter.Sql, $"Phase {phaseId} defined in `phase_area` does not exist, skipped.");
                    continue;
                }
        ...
        public void LoadNPCSpellClickSpells()
        {
            ...
            do
            {
                ...
                if (cInfo == null)
                {
                    Log.outWarn(LogFilter.Sql, "Table npc_spellclick_spells references unknown creature_template {0}. Skipping entry.", npc_entry);
                    continue;
                }
                ...
                if (spellinfo == null)
                {
                    Log.outWarn(LogFilter.Sql, "Table npc_spellclick_spells creature: {0} references unknown spellid {1}. Skipping entry.", npc_entry, spellid);
                    continue;
                }

                SpellClickUserTypes userType = (SpellClickUserTypes)result.Read<byte>(3);
                if (userType >= SpellClickUserTypes.Max)
                    Log.outWarn(LogFilter.Sql, "Table npc_spellclick_spells creature: {0} references unknown user type {1}. Skipping entry.", npc_entry, userType);
        ...
        public void LoadFishingBaseSkillLevel()
        {
            ...
            do
            {
                ...
                if (fArea == null)
                {
                    Log.outWarn(LogFilter.Sql, "AreaId {0} defined in `skill_fishing_base_level` does not exist", entry);
                    continue;
                }
            ...
            do
            {
                ...
                switch (type)
                {
                    case 0: // Creature
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, $"Table `quest_greeting_locale`: creature template entry {id} does not exist.");
                            continue;
                        }
                        break;
                    case 1: // GameObject
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, $"Table `quest_greeting_locale`: gameobject template entry {id} does not exist.");
                            continue;
                        }
                        break;
                    default:
        ...
        public void LoadReputationRewardRate()
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Faction (faction.dbc) {0} does not exist but is used in `reputation_reward_rate`", factionId);
                    continue;
                }

                if (repRate.questRate < 0.0f)
                {
                    Log.outWarn(LogFilter.Sql, "Table reputation_reward_rate has quest_rate with invalid rate {0}, skipping data for faction {1}", repRate.questRate, factionId);
                    continue;
                }

                if (repRate.questDailyRate < 0.0f)
                {
                    Log.outWarn(LogFilter.Sql, "Table reputation_reward_rate has quest_daily_rate with invalid rate {0}, skipping data for faction {1}", repRate.questDailyRate, factionId);
                    continue;
                }

                if (repRate.questWeeklyRate < 0.0f)
                {
                    Log.outWarn(LogFilter.Sql, "Table reputation_reward_rate has quest_weekly_rate with invalid rate {0}, skipping data for faction {1}", repRate.questWeeklyRate, factionId);
                    continue;
                }

                if (repRate.questMonthlyRate < 0.0f)
                {
                    Log.outWarn(LogFilter.Sql, "Table reputation_reward_rate has quest_monthly_rate with invalid rate {0}, skipping data for faction {1}", repRate.questMonthlyRate, factionId);
                    continue;
                }

                if (repRate.questRepeatableRate < 0.0f)
                {
                    Log.outWarn(LogFilter.Sql, "Table reputation_reward_rate has quest_repeatable_rate with invalid rate {0}, skipping data for faction {1}", repRate.questRepeatableRate, factionId);
                    continue;
                }

                if (repRate.creatureRate < 0.0f)
                {
                    Log.outWarn(LogFilter.Sql, "Table reputation_reward_rate has creature_rate with invalid rate {0}, skipping data for faction {1}", repRate.creatureRate, factionId);
                    continue;
                }

                if (repRate.spellRate < 0.0f)
                {
                    Log.outWarn(LogFilter.Sql, "Table reputation_reward_rate has spell_rate with invalid rate {0}, skipping data for faction {1}", repRate.spellRate, factionId);
                    continue;
                }
        ...
        public void LoadReputationOnKill()
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Table `creature_onkill_reputation` have data for not existed creature entry ({0}), skipped", creature_id);
                    continue;
                }
                ...
                if (repOnKill.RepFaction1 != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Faction (faction.dbc) {0} does not exist but is used in `creature_onkill_reputation`", repOnKill.RepFaction1);
                        continue;
                    }
                }
                ...
                if (repOnKill.RepFaction2 != 0)
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, "Faction (faction.dbc) {0} does not exist but is used in `creature_onkill_reputation`", repOnKill.RepFaction2);
                        continue;
                    }
                }
        ...
        public void LoadReputationSpilloverTemplate()
        {
            ...
            do
            {
                ...
                if (factionEntry == null)
                {
                    Log.outWarn(LogFilter.Sql, "Faction (faction.dbc) {0} does not exist but is used in `reputation_spillover_template`", factionId);
                    continue;
                }

                if (factionEntry.ParentFactionID == 0)
                {
                    Log.outWarn(LogFilter.Sql, "Faction (faction.dbc) {0} in `reputation_spillover_template` does not belong to any team, skipping", factionId);
                    continue;
                }
                ...
                for (var i = 0; i < 5; ++i)
                {
                    if (repTemplate.faction[i] != 0)
                    {
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Spillover faction (faction.dbc) {0} does not exist but is used in `reputation_spillover_template` for faction {1}, skipping", repTemplate.faction[i], factionId);
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Spillover faction (faction.dbc) {0} for faction {1} in `reputation_spillover_template` can not be listed for client, and then useless, skipping",
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Rank {0} used in `reputation_spillover_template` for spillover faction {1} is not valid, skipping", repTemplate.faction_rank[i], repTemplate.faction[i]);
                            invalidSpilloverFaction = true;
                            break;
                        }
                    }
        ...
        public void LoadMailLevelRewards()
        {
            ...
            do
            {
                ...
                if (level > WorldConfig.GetIntValue(WorldCfg.MaxPlayerLevel))
                {
                    Log.outWarn(LogFilter.Sql, "Table `mail_level_reward` have data for level {0} that more supported by client ({1}), ignoring.", level, WorldConfig.GetIntValue(WorldCfg.MaxPlayerLevel));
                    continue;
                }

                if (!Convert.ToBoolean(raceMask & SharedConst.RaceMaskAllPlayable))
                {
                    Log.outWarn(LogFilter.Sql, "Table `mail_level_reward` have raceMask ({0}) for level {1} that not include any player races, ignoring.", raceMask, level);
                    continue;
                }

                if (!CliDB.MailTemplateStorage.ContainsKey(mailTemplateId))
                {
                    Log.outWarn(LogFilter.Sql, "Table `mail_level_reward` have invalid mailTemplateId ({0}) for level {1} that invalid not include any player races, ignoring.", mailTemplateId, level);
                    continue;
                }

                if (GetCreatureTemplate(senderEntry) == null)
                {
                    Log.outWarn(LogFilter.Sql, "Table `mail_level_reward` have not existed sender creature entry ({0}) for level {1} that invalid not include any player races, ignoring.", senderEntry, level);
                    continue;
                }
            ...
            do
            {
                ...
                switch (summonerType)
                {
                    case SummonerType.Creature:
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `creature_summon_groups` has summoner with non existing entry {0} for creature summoner type, skipped.", summonerId);
                            continue;
                        }
                        break;
                    case SummonerType.GameObject:
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `creature_summon_groups` has summoner with non existing entry {0} for gameobject summoner type, skipped.", summonerId);
                            continue;
                        }
                        break;
                    case SummonerType.Map:
                        ...
                        {
                            Log.outWarn(LogFilter.Sql, "Table `creature_summon_groups` has summoner with non existing entry {0} for map summoner type, skipped.", summonerId);
                            continue;
                        }
                        break;
                    default:
                        Log.outWarn(LogFilter.Sql, "Table `creature_summon_groups` has unhandled summoner type {0} for summoner {1}, skipped.", summonerType, summonerId);
                        continue;
                }
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Table `creature_summon_groups` has creature in group [Summoner ID: {0}, Summoner Type: {1}, Group ID: {2}] with non existing creature entry {3}, skipped.",
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Table `creature_summon_groups` has unhandled temp summon type {0} in group [Summoner ID: {1}, Summoner Type: {2}, Group ID: {3}] for creature entry {4}, skipped.",
            ...
            foreach (var pair in _pageTextStorage)
            {
                if (pair.Value.NextPageID != 0)
                {
                    if (!_pageTextStorage.ContainsKey(pair.Value.NextPageID))
                        Log.outWarn(LogFilter.Sql, "Page text (ID: {0}) has non-existing `NextPageID` ({1})", pair.Key, pair.Value.NextPageID);
        ...
        public void LoadPlayerChoices()
        {
            ...
            if (!responses.IsEmpty())
            {
                do
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_response` references non-existing ChoiceId: {choiceId} (ResponseId: {responseId}), skipped");
                        continue;
                    }
            ...
            if (!rewards.IsEmpty())
            {
                do
                {
                    ...
                    if (!_playerChoices.ContainsKey(choiceId))
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_response_reward` references non-existing ChoiceId: {choiceId} (ResponseId: {responseId}), skipped");
                        continue;
                    }
                    ...
                    if (response == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_response_reward` references non-existing ResponseId: {responseId} for ChoiceId {choiceId}, skipped");
                        continue;
                    }
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_response_reward` references non-existing Title {reward.TitleId} for ChoiceId {choiceId}, ResponseId: {responseId}, set to 0");
                        reward.TitleId = 0;
                    }

                    if (reward.PackageId != 0 && Global.DB2Mgr.GetQuestPackageItems((uint)reward.PackageId) == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_response_reward` references non-existing QuestPackage {reward.TitleId} for ChoiceId {choiceId}, ResponseId: {responseId}, set to 0");
                        reward.PackageId = 0;
                    }

                    if (reward.SkillLineId != 0 && !CliDB.SkillLineStorage.ContainsKey(reward.SkillLineId))
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_response_reward` references non-existing SkillLine {reward.TitleId} for ChoiceId {choiceId}, ResponseId: {responseId}, set to 0");
            ...
            if (!rewardItem.IsEmpty())
            {
                do
                {
                    ...
                    if (choice == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_response_reward_item` references non-existing ChoiceId: {choiceId} (ResponseId: {responseId}), skipped");
                        continue;
                    }
                    ...
                    if (response == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_response_reward_item` references non-existing ResponseId: {responseId} for ChoiceId {choiceId}, skipped");
                        continue;
                    }

                    if (response.Reward == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_response_reward_item` references non-existing player choice reward for ChoiceId {choiceId}, ResponseId: {responseId}, skipped");
                        continue;
                    }

                    if (GetItemTemplate(itemId) == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_response_reward_item` references non-existing item {itemId} for ChoiceId {choiceId}, ResponseId: {responseId}, skipped");
                        continue;
                    }
            ...
            if (!rewardCurrency.IsEmpty())
            {
                do
                {
                    ...
                    if (choice == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_response_reward_currency` references non-existing ChoiceId: {choiceId} (ResponseId: {responseId}), skipped");
                        continue;
                    }
                    ...
                    if (response == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_response_reward_currency` references non-existing ResponseId: {responseId} for ChoiceId {choiceId}, skipped");
                        continue;
                    }

                    if (response.Reward == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_response_reward_currency` references non-existing player choice reward for ChoiceId {choiceId}, ResponseId: {responseId}, skipped");
                        continue;
                    }

                    if (!CliDB.CurrencyTypesStorage.ContainsKey(currencyId))
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_response_reward_currency` references non-existing currency {currencyId} for ChoiceId {choiceId}, ResponseId: {responseId}, skipped");
                        continue;
                    }
            ...
            if (!rewardFaction.IsEmpty())
            {
                do
                {
                    ...
                    if (choice == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_response_reward_faction` references non-existing ChoiceId: {choiceId} (ResponseId: {responseId}), skipped");
                        continue;
                    }
                    ...
                    if (response == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_response_reward_faction` references non-existing ResponseId: {responseId} for ChoiceId {choiceId}, skipped");
                        continue;
                    }

                    if (response.Reward == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_response_reward_faction` references non-existing player choice reward for ChoiceId {choiceId}, ResponseId: {responseId}, skipped");
                        continue;
                    }

                    if (!CliDB.FactionStorage.ContainsKey(factionId))
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_response_reward_faction` references non-existing faction {factionId} for ChoiceId {choiceId}, ResponseId: {responseId}, skipped");
                        continue;
                    }
            ...
            if (!rewardItems.IsEmpty())
            {
                do
                {
                    ...
                    if (choice == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_response_reward_item_choice` references non-existing ChoiceId: {choiceId} (ResponseId: {responseId}), skipped");
                        continue;
                    }
                    ...
                    if (response == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_response_reward_item_choice` references non-existing ResponseId: {responseId} for ChoiceId {choiceId}, skipped");
                        continue;
                    }

                    if (response.Reward == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_response_reward_item_choice` references non-existing player choice reward for ChoiceId {choiceId}, ResponseId: {responseId}, skipped");
                        continue;
                    }

                    if (GetItemTemplate(itemId) == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_response_reward_item_choice` references non-existing item {itemId} for ChoiceId {choiceId}, ResponseId: {responseId}, skipped");
                        continue;
                    }
            ...
            if (!mawPowersResult.IsEmpty())
            {
                do
                {
                    ...
                    if (choice == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_response_maw_power` references non-existing ChoiceId: {choiceId} (ResponseId: {responseId}), skipped");
                        continue;
                    }
                    ...
                    if (response == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_response_maw_power` references non-existing ResponseId: {responseId} for ChoiceId {choiceId}, skipped");
                        continue;
                    }
        ...
        public void LoadPlayerChoicesLocale()
        {
            ...
            if (!result.IsEmpty())
            {
                do
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_locale` references non-existing ChoiceId: {choiceId} for locale {localeName}, skipped");
                        continue;
                    }
            ...
            if (!result.IsEmpty())
            {
                ...
                do
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_locale` references non-existing ChoiceId: {choiceId} for ResponseId {responseId} locale {localeName}, skipped");
                        continue;
                    }
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `playerchoice_locale` references non-existing ResponseId: {responseId} for ChoiceId {choiceId} locale {localeName}, skipped");
                        continue;
                    }
        ...
        public void LoadJumpChargeParams()
        {
            ...
            do
            {
                ...
                if (speed <= 0.0f)
                {
                    Log.outWarn(LogFilter.Sql, $"Table `jump_charge_params` uses invalid speed {speed} for id {id}, set to default charge speed {MotionMaster.SPEED_CHARGE}.");
                    speed = MotionMaster.SPEED_CHARGE;
                }

                if (jumpGravity <= 0.0f)
                {
                    Log.outWarn(LogFilter.Sql, $"Table `jump_charge_params` uses invalid jump gravity {jumpGravity} for id {id}, set to default {MotionMaster.gravity}.");
                ...
                if (!result.IsNull(4))
                {
                    if (CliDB.SpellVisualStorage.ContainsKey(result.Read<uint>(4)))
                        ...
                        Log.outWarn(LogFilter.Sql, $"Table `jump_charge_params` references non-existing SpellVisual: {result.Read<uint>(4)} for id {id}, ignored.");
                }
                ...
                if (!result.IsNull(5))
                {
                    if (CliDB.CurveStorage.ContainsKey(result.Read<uint>(5)))
                        ...
                        Log.outWarn(LogFilter.Sql, $"Table `jump_charge_params` references non-existing progress Curve: {result.Read<uint>(5)} for id {id}, ignored.");
                }
                ...
                if (!result.IsNull(6))
                {
                    if (CliDB.CurveStorage.ContainsKey(result.Read<uint>(6)))
                        ...
                        Log.outWarn(LogFilter.Sql, $"Table `jump_charge_params` references non-existing parabolic Curve: {result.Read<uint>(6)} for id {id}, ignored.");
                }
        ...
        public uint GetTaxiMountDisplayId(uint id, Team team, bool allowed_alt_team = false)
        {
            ...
            if (node != null)
            {
                ...
                if (mount_info != null)
                {
                    ...
                    if (model == null)
                    {
                        Log.outWarn(LogFilter.Sql, $"No displayid found for the taxi mount with the entry {mount_entry}! Can't load it!");
                        return 0;
                    }
                    mountModel = model;
                }
        ...
        public void LoadVehicleTemplate()
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"Table `vehicle_template`: Vehicle {creatureId} does not exist.");
                    continue;
                }
        ...
        public void LoadVehicleTemplateAccessories()
        {
            ...
            do
            {
                ...
                if (GetCreatureTemplate(entry) == null)
                {
                    Log.outWarn(LogFilter.Sql, "Table `vehicle_template_accessory`: creature template entry {0} does not exist.", entry);
                    continue;
                }

                if (GetCreatureTemplate(accessory) == null)
                {
                    Log.outWarn(LogFilter.Sql, "Table `vehicle_template_accessory`: Accessory {0} does not exist.", accessory);
                    continue;
                }

                if (!_spellClickInfoStorage.ContainsKey(entry))
                {
                    Log.outWarn(LogFilter.Sql, "Table `vehicle_template_accessory`: creature template entry {0} has no data in npc_spellclick_spells", entry);
                    continue;
                }
        ...
        public void LoadVehicleAccessories()
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, "Table `vehicle_accessory`: Accessory {0} does not exist.", uiAccessory);
                    continue;
                }
        ...
        public void LoadVehicleSeatAddon()
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"Table `vehicle_seat_addon`: SeatID: {seatID} does not exist in VehicleSeat.dbc. Skipping entry.");
                    continue;
                }
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"Table `vehicle_seat_addon`: SeatID: {seatID} is using invalid angle offset value ({orientation}). Set Value to 0.");
                    orientation = 0.0f;
                }

                if (exitParam >= (byte)VehicleExitParameters.VehicleExitParamMax)
                {
                    Log.outWarn(LogFilter.Sql, $"Table `vehicle_seat_addon`: SeatID: {seatID} is using invalid exit parameter value ({exitParam}). Setting to 0 (none).");
                    continue;
                }
    ...
    public class PetLevelInfo
    {
        ...
        public uint[] stats = new uint[(int)CharacterStats.Max];
        public uint health;
        public uint mana;
        public uint armor;
    }
