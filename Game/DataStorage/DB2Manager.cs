    ...
    public class DB2Manager : Singleton<DB2Manager>
    {
        ...
        public void LoadHotfixData()
        {
            ...
            do
            {
                ...
                if (status == HotfixRecord.Status.Valid && !_storage.ContainsKey(tableHash))
                {
                    ...
                    {
                        Log.outWarn(LogFilter.Sql, $"Table `hotfix_data` references unknown DB2 store by hash 0x{tableHash:X} and has no reference to `hotfix_blob` in hotfix id {id} with RecordID: {recordId}");
                        continue;
                    }
                }
        ...
        public void LoadHotfixBlob(BitSet availableDb2Locales)
        {
            ...
            do
            {
                ...
                if (storeItr != null)
                {
                    Log.outWarn(LogFilter.Sql, $"Table hash 0x{tableHash:X} points to a loaded DB2 store {storeItr.GetName()}, fill related table instead of hotfix_blob");
                    continue;
                }
        ...
        public void LoadHotfixOptionalData(BitSet availableDb2Locales)
        {
            ...
            do
            {
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"Table `hotfix_optional_data` references DB2 store by hash 0x{tableHash:X} that is not allowed to have optional data");
                    continue;
                }
                ...
                if (db2storage == null)
                {
                    Log.outWarn(LogFilter.Sql, $"Table `hotfix_optional_data` references unknown DB2 store by hash 0x{tableHash:X} with RecordID: {recordId}");
                    continue;
                }
                ...
                if (!SharedConst.IsValidLocale(locale))
                {
                    Log.outWarn(LogFilter.Sql, $"`hotfix_optional_data` contains invalid locale: {localeName} at TableHash: 0x{tableHash:X} and RecordID: {recordId}");
                    continue;
                }
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"Table `hotfix_optional_data` references non-allowed optional data key 0x{optionalData.Key:X} for DB2 store by hash 0x{tableHash:X} and RecordID: {recordId}");
                    continue;
                }
                ...
                {
                    Log.outWarn(LogFilter.Sql, $"Table `hotfix_optional_data` contains invalid data for DB2 store 0x{tableHash:X}, RecordID: {recordId} and Key: 0x{optionalData.Key:X}");
                    continue;
                }
