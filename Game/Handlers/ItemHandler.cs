    ...
    public partial class WorldSession
    {
        ...
        void HandleSocketGems(SocketGems socketGems)
        {
            ...
            for (uint i = 0; i < ItemConst.MaxGemSockets; ++i)                //check for hack maybe
            {
                ...
                if (itemTarget.GetSocketColor(i) == SocketColor.Meta && SocketColor.Meta != gemProperties[i].Type)
                    return;

                if (itemTarget.GetSocketColor(i) != SocketColor.Meta && SocketColor.Meta == gemProperties[i].Type)
                    return;

                if (itemTarget.GetSocketColor(i) == SocketColor.Cogwheel && SocketColor.Cogwheel != gemProperties[i].Type)
                    return;
